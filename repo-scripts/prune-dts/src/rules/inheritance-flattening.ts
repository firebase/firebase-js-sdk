...
function flattenClassInheritance(
  ...
  } else if (!baseClass && cls.getExtends()) {
    // When API Extractor strips @internal base declarations, getBaseClass() returns undefined.
...
function buildTypeArgMap(
  ...
  const baseName = Node.isNameable(baseDecl) ? baseDecl.getName() : undefined;
  const targetName = Node.isNameable(targetDecl)
    ? targetDecl.getName()
    : undefined;
  ...
  const typeParams = Node.isTypeParametered(baseDecl)
    ? baseDecl.getTypeParameters()
    : [];
  ...
  for (const clause of clauses) {
    ...
    if (text.split('<')[0].trim() === baseName) {
      const typeArgs = Node.isTypeArgumented(clause)
        ? clause.getTypeArguments()
        : [];
      for (let i = 0; i < typeParams.length; i++) {
        if (i < typeArgs.length) {
          map.set(typeParams[i].getName(), typeArgs[i].getText().trim());
        } else {
          const defaultNode = typeParams[i].getDefault();
          if (defaultNode) {
...
function copyDeclarationMembers(
  ...
  const isTargetClass = targetDecl.getKind() === SyntaxKind.ClassDeclaration;
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sourceMembers: Node[] =
    typeof (sourceDecl as any).getMembers === 'function'
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (sourceDecl as any).getMembers()
      : // eslint-disable-next-line @typescript-eslint/no-explicit-any
        typeof (sourceDecl as any).getProperties === 'function' &&
          typeof (sourceDecl as any).getMethods === 'function'
        ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
          [
            ...(sourceDecl as any).getProperties(),
            ...(sourceDecl as any).getMethods()
          ]
        : [
  ...
  for (const member of sourceMembers) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (
      typeof (member as any).getScope === 'function' &&
      (member as any).getScope() === 'private'
    ) {
      continue;
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const name =
      typeof (member as any).getName === 'function'
        ? (member as any).getName()
        : null;
 ...
 */
/* eslint-disable @typescript-eslint/no-explicit-any */
function substituteStructureTypeArgs(
  structure: any,
  ...
  if (structure.type) {
    structure.type = replaceStr(structure.type);
  }
  if (structure.returnType) {
    structure.returnType = replaceStr(structure.returnType);
  }
  ...
  if (Array.isArray(structure.parameters)) {
    ...
    for (const param of structure.parameters) {
      if (param.type) {
        param.type = replaceStr(param.type);
      }
    }
  }
}
...
function isNodeExported(node: Node): boolean {
  ...
  if (Node.isExportable(node) && node.hasDefaultKeyword()) {
    return true;
  }
  const name = Node.isNameable(node) ? node.getName() : undefined;
  if (name) {
    const sourceFile = node.getSourceFile();
    const exportDecls = sourceFile.getDescendantsOfKind(
  ...
  if (parent && parent.getKind() === SyntaxKind.ModuleBlock) {
    return Node.isExportable(node) && node.hasExportKeyword();
  }
  return false;
}
...
function findDeclarationByName(
  ...
  for (const cls of classes) {
    if (cls.getName() === cleanName) {
      return cls;
    }
  }
  const interfaces = sourceFile.getInterfaces();
  for (const iface of interfaces) {
    if (iface.getName() === cleanName) {
      return iface;
    }
  }
  const typeAliases = sourceFile.getTypeAliases();
  for (const ta of typeAliases) {
    if (ta.getName() === cleanName) {
      return ta;
    }
  }
  return undefined;
 ...
 */
function shouldIncludeAsPublicBase(
  name: string,
  sourceFile: SourceFile
): boolean {
  const cleanName = name.trim().split('<')[0].trim();
