...
import { extractDeclarations, MemberList, dedup, mapSymbolToType, replaceAll, writeReportToFile, ErrorCode, writeReportToDirectory } from './analysis-helper';
...
describe('test dedup helper function', () => {
  ...
  it('test dedup with non-empty entries', () => {
    ...
    expect(memberList.functions.filter(each => each.localeCompare("aFunc") === 0).length).to.equal(1);
    expect(memberList.classes.filter(each => each.localeCompare("aClass") === 0).length).to.equal(1);
    expect(memberList.variables.filter(each => each.localeCompare("aVar") === 0).length).to.equal(1);
    expect(memberList.enums.filter(each => each.localeCompare("aEnum") === 0).length).to.equal(1);
  });
...
describe('test replaceAll helper function', () => {
  it('test replaceAll with multiple occurences of an element', () => {
    ...
    expect(memberList.functions.filter(each => each.localeCompare(original) === 0).length).to.equal(0);
    expect(memberList.functions.filter(each => each.localeCompare(replaceTo) === 0).length).to.equal(2);
  ...
  it('test replaceAll with single occurence of an element', () => {
    ...
    expect(memberList.classes.filter(each => each.localeCompare(original) === 0).length).to.equal(0);
    expect(memberList.classes.filter(each => each.localeCompare(replaceTo) === 0).length).to.equal(1);
  });
  ...
  it('test replaceAll with zero occurence of an element', () => {
    ...
    expect(memberList.enums).to.have.length(4);
    expect(memberList.enums.filter(each => each.localeCompare(original) === 0).length).to.equal(0);
    expect(memberList.enums.filter(each => each.localeCompare(replaceTo) === 0).length).to.equal(0);
  });
...
describe('test mapSymbolToType helper function', () => {
  ...
  it('test if function correctly categorizes symbols that are misplaced', () => {
    ...
    expect(memberList.enums).to.have.members(["aEnum", "bEnum", "cEnum", "dEnum"]);
...
describe('test writeReportToFile helper function', () => {
  ...
  it('should throw error when given path exists and points to directory', () => {
    ...
    expect(() => writeReportToFile("content", aDir)).to.throw(ErrorCode.OUTPUT_FILE_REQUIRED);
  });
