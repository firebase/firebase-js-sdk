...
function makeStorage(url: string) {
  ...
  const authWrapper = new AuthWrapper(
      null, maker, makeRequest, ({} as any) as Service, testShared.makePool(null));
...
describe("Firebase Storage > Reference", () => {
  ...
  describe("child", () => {
    ...
    it("compresses repeated slashes", () => {
      assert.equal(root.child('//a///b/////').toString(), 'gs://test-bucket/a/b');
    });
    it("works chained multiple times with leading slashes", () => {
      assert.equal(root.child('a').child('/b').child('c').child('d/e').toString(),
  ...
  it("Works if the user logs in before creating the storage reference", (done) => {
    ...
    function newSend(xhrio: TestingXhrIo, url: string, method: string, body?: ArrayBufferView|Blob|string|null, headers?: Headers) {
      assert.isDefined(headers);
      assert.equal(headers['Authorization'], 'Firebase ' + testShared.authToken);
      done();
    }
  ...
  describe("putString", () => {
    it("Uses metadata.contentType for RAW format", () => {
      ...
      const task = child.putString('hello', StringFormat.RAW, {'contentType': 'lol/wut'} as Metadata);
      assert.equal(task.snapshot.metadata.contentType, 'lol/wut');
      task.cancel();
    });
    it("Uses embedded content type in DATA_URL format", () => {
      const task = child.putString('data:lol/wat;base64,aaaa', StringFormat.DATA_URL);
  ...
  describe("Argument verification", () => {
    describe("child", () => {
      it("throws on no args", () => {
        testShared.assertThrows(testShared.bind(root.child, root), 'storage/invalid-argument-count');
      });
      it("throws on null instead of path", () => {
        testShared.assertThrows(testShared.bind(root.child, root, null), 'storage/invalid-argument');
      });
      it("throws on number instead of path", () => {
        testShared.assertThrows(testShared.bind(root.child, root, 3), 'storage/invalid-argument');
      });
    });

    describe("toString", () => {
      it("throws on number arg", () => {
        testShared.assertThrows(testShared.bind(root.toString, root, 3), 'storage/invalid-argument-count');
      });
    });
    ...
    describe("put", () => {
      ...
      it("throws on no arguments", () => {
        testShared.assertThrows(testShared.bind(child.put, child), 'storage/invalid-argument-count');
      });
      it("throws on number instead of metadata", () => {
        testShared.assertThrows(testShared.bind(child.put, child, new Blob([]), 3), 'storage/invalid-argument');
      });
      it("throws on number instead of data", () => {
        testShared.assertThrows(testShared.bind(child.put, child, 3), 'storage/invalid-argument');
      });
      it("throws null instead of data", () => {
        testShared.assertThrows(testShared.bind(child.put, child, null), 'storage/invalid-argument');
      });
      ...
      it("throws when customMetadata is a string instead of an object", () => {
        ...
        testShared.assertThrows(testShared.bind(child.put, child, blob, badCustomMetadata), 'storage/invalid-argument');
      });
      it("throws when object is supplied instead of string", () => {
        ...
        testShared.assertThrows(testShared.bind(child.put, child, blob, objectInsteadOfStringInMetadata), 'storage/invalid-argument');
      });
    });

    describe("putString", () => {
      it("throws on no arguments", () => {
        testShared.assertThrows(testShared.bind(child.putString, child), 'storage/invalid-argument-count');
      });
      it("throws on invalid format", () => {
        testShared.assertThrows(testShared.bind(child.putString, child, 'raw', 'notaformat'), 'storage/invalid-argument');
      });
      it("throws on number instead of string", () => {
        testShared.assertThrows(testShared.bind(child.putString, child, 3, StringFormat.RAW), 'storage/invalid-argument');
      });
      it("throws on invalid metadata", () => {
        testShared.assertThrows(testShared.bind(child.putString, child, 'raw', StringFormat.RAW, 3), 'storage/invalid-argument');
      });
    });

    describe("delete", () => {
      it("throws on a number arg", () => {
        testShared.assertThrows(testShared.bind(child.delete, child, 3), 'storage/invalid-argument-count');
      });
    });

    describe("getMetadata", () => {
      it("throws on a number arg", () => {
        testShared.assertThrows(testShared.bind(child.getMetadata, child, 3), 'storage/invalid-argument-count');
      });
    });

    describe("updateMetadata", () => {
      it("throws on no args", () => {
        testShared.assertThrows(testShared.bind(child.updateMetadata, child), 'storage/invalid-argument-count');
      });
      it("throws on number arg", () => {
        testShared.assertThrows(testShared.bind(child.updateMetadata, child, 3), 'storage/invalid-argument');
      });
      it("throws on null arg", () => {
        testShared.assertThrows(testShared.bind(child.updateMetadata, child, null), 'storage/invalid-argument');
      });
    });

    describe("getDownloadURL", () => {
      it("throws on number arg", () => {
        testShared.assertThrows(testShared.bind(child.getDownloadURL, child, 3), 'storage/invalid-argument-count');
      });
    });
  });
  ...
  describe("non-root operations", () => {
    ...
    it("putString doesn't throw", () => {
      assert.doesNotThrow(() => {
        ...
        child.putString('data:application/octet-stream;base64,aaaa', StringFormat.DATA_URL);
      });
    });
  ...
  describe("root operations", () => {
    it("put throws", () => {
      testShared.assertThrows(root.put.bind(root, new Blob(['a'])), 'storage/invalid-root-operation');
    });
    it("putString throws", () => {
      testShared.assertThrows(root.putString.bind(root, 'raw', StringFormat.RAW), 'storage/invalid-root-operation');
    });
    it("delete throws", () => {
      testShared.assertThrows(root.delete.bind(root), 'storage/invalid-root-operation');
    });
    it("getMetadata throws", () => {
      testShared.assertThrows(root.getMetadata.bind(root), 'storage/invalid-root-operation');
    });
    it("updateMetadata throws", () => {
      testShared.assertThrows(root.updateMetadata.bind(root, {}), 'storage/invalid-root-operation');
    });
    it("getDownloadURL throws", () => {
      testShared.assertThrows(root.getDownloadURL.bind(root), 'storage/invalid-root-operation');
    });
  });
});
