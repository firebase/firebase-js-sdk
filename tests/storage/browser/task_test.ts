...
import {TaskEvent, TaskState} from '../../../src/storage/implementation/taskenums';
...
type RequestHandler = (url: string, method: string, body?: ArrayBufferView|Blob|string, headers?: Headers) => Response;

function authWrapperWithHandler(handler: RequestHandler): AuthWrapper {
  function newSend(xhrio: TestingXhrIo, url: string, method: string, body?: ArrayBufferView|Blob|string, headers?: Headers): void {
    const response = handler(url, method, body, headers);
    xhrio.simulateResponse(response.status, response.body, response.headers);
  }

  return new AuthWrapper(null, (_1, _2) => { return {} as Reference; }, makeRequest, {} as Service, makePool(newSend));
}
...
function fakeServerHandler(): RequestHandler {
  ...
  function statusHeaders(status: string, opt_existing?: StringHeaders): StringHeaders {
    if (opt_existing) {
  ...
  function handler(url: string, method: string, content?: ArrayBufferView|Blob|string, headers?: Headers): Response {
    method = method || 'GET';
    content = content || '';
    headers = headers || {};

    if (headers['X-Goog-Upload-Protocol'] === 'multipart') {
      return {status: 200, body: fakeMetadata, headers: statusHeaders('final')};
    }

    const contentLength = (content as Blob).size || (content as string).length || 0;
    ...
    if (headers['X-Goog-Upload-Command'] === 'query') {
      return {
        ...
        body: '',
        headers: statusHeaders(
            'active', {'X-Goog-Upload-Size-Received': stats[id].currentSize.toString()})
      };
    }

    const commands = (headers['X-Goog-Upload-Command'] as string).split(',').map(str => { return str.trim(); });
...
describe("Firebase Storage > Upload Task", () => {
  ...
  it("Works for a small upload canceled w/ a promise", () => {
    ...
    const promise: Promise<string> = task.then<string>(
        snapshot => { assert.fail('task completed, but should have failed'); return null; },
  ...
  function runNormalUploadTest(blob: FbsBlob): Promise<void> {
    const authWrapper = authWrapperWithHandler(fakeServerHandler());
    const task = new UploadTask(
        {} as Reference, authWrapper, testLocation, mappings, blob);
  ...
  describe("Argument verification", () => {
    ...
    describe("on", () => {
      it("Throws on no args", () => {
        assertThrows(
            fbsBind(task.on, task), 'storage/invalid-argument-count');
      });
      it("Throws on 5 args", () => {
        assertThrows(fbsBind(
                task.on, task, TaskEvent.STATE_CHANGED, null, null, null, 1),
            'storage/invalid-argument-count');
      });
      it("Throws on a single string arg", () => {
        assertThrows(
            fbsBind(task.on, task, '3'), 'storage/invalid-argument');
      });
      it("Throws on a single null arg", () => {
        assertThrows(
            fbsBind(task.on, task, null), 'storage/invalid-argument');
      });
    ...
    describe("subscribe returned from on", () => {
      ...
      it("Throws on 4 args", () => {
        assertThrows(fbsBind(task.on(TaskEvent.STATE_CHANGED), null, null, null, null, 1),
            'storage/invalid-argument-count');
    ...
    describe("resume", () => {
      it("Throws on a number", () => {
        assertThrows(fbsBind(task.resume, task, 3), 'storage/invalid-argument-count');
      });
    });
    describe("pause", () => {
      it("Throws on a number", () => {
        assertThrows(
            fbsBind(task.pause, task, 3), 'storage/invalid-argument-count');
      });
    });
    describe("cancel", () => {
      it("Throws on a number", () => {
        assertThrows(
            fbsBind(task.cancel, task, 3), 'storage/invalid-argument-count');
      });
    });
  });
});
