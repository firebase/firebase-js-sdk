...
describe('Firebase Messaging > tokenManager.createToken()', function() {
  ...
  it('should handle failing getSubscription', function() {
    ...
    globalTokenManager = new TokenManager();
    return globalTokenManager.createToken(EXAMPLE_SENDER_ID,
  ...
  it('should handle failing subscribe call', function() {
    ...
    const subscribeStub = sinon.stub(TokenManager.prototype, 'subscribeToFCM')
  ...
  it('should handle valid flow', function() {
    ...
    const installToActivateReg = makeFakeSWReg(
      'installing',
      swValue
    );
    ...
    return validCombos.reduce((promiseChain, validCombo) => {
      return promiseChain.then(() => {
        return globalTokenManager.createToken(validCombo.senderId,
          validCombo.swReg)
        .then(token => {
  ...
  it('should handle sender ID difference', function() {
    ...
    methodStub.withArgs(EXAMPLE_SENDER_ID, EXAMPLE_SUBSCRIPTION).returns(Promise.resolve(EXAMPLE_FCM_TOKEN_DETAILS));
    methodStub.withArgs(secondSenderId, EXAMPLE_SUBSCRIPTION).returns(Promise.resolve(secondDetails))
