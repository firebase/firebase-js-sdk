...
describe('CompoundWrite Tests', function() {
  ...
  const CHILDREN_NODE = nodeFromJSON({ 'child-1': 'value-1', 'child-2': 'value-2' });
  ...
  it('CompoundWrite with root priority update, child write is not empty.', function() {
    let compoundWrite = CompoundWrite.Empty.addWrite(new Path('.priority'), PRIO_NODE);
  ...
  it('shallow update removes deep update', function() {
    ...
    const expected = CHILDREN_NODE.updateImmediateChild('child-1',
  ...
  it('adding updates at path works', function() {
    ...
    const expected = CHILDREN_NODE.updateImmediateChild('child-1', nodeFromJSON(expectedChildOne));
    assertNodesEqual(expected, compoundWrite.apply(CHILDREN_NODE));
  });
  ...
  it('compound write with deep update has shadowing write', function() {
    let compoundWrite = CompoundWrite.Empty.addWrite(new Path('deep/update'), LEAF_NODE);
