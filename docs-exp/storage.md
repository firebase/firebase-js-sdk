...
|  --- | --- |
|  [StorageObserver](./storage.storageobserver.md) | An observer that wraps any errors in <code>FirebaseStorageError</code>. |

## Variables

|  Variable | Description |
|  --- | --- |
|  [StorageErrorCode](./storage.storageerrorcode.md) | Error codes that can be attached to <code>FirebaseStorageError</code>s. |
...
|  --- | --- |
|  [CompleteFn](./storage.completefn.md) | A function that is called if the event stream ends normally. |
|  [ErrorFn](./storage.errorfn.md) | A function that is called with a <code>FirebaseStorageError</code> if the event stream ends due to an error. |
|  [NextFn](./storage.nextfn.md) | Function that is called once for each value in a stream of values. |
|  [StorageErrorCode](./storage.storageerrorcode.md) | Error codes that can be attached to <code>FirebaseStorageError</code>s. |
|  [StringFormat](./storage.stringformat.md) | An enumeration of the possible string formats for upload. |
|  [Subscribe](./storage.subscribe.md) | Subscribes to an event stream. |
|  [TaskEvent](./storage.taskevent.md) | An event that is triggered on a task. |
|  [TaskState](./storage.taskstate.md) | Represents the current state of a running upload. |
|  [Unsubscribe](./storage.unsubscribe.md) | Unsubscribes from a stream. |
