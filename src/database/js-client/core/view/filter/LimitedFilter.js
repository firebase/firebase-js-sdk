...
fb.core.view.filter.LimitedFilter = function(params) {
  /**
   * @const
   * @type {fb.core.view.filter.RangedFilter}
   * @private
   */
 ...
 * @inheritDoc
 */
fb.core.view.filter.LimitedFilter.prototype.updateChild = function(snap, key, newChild, affectedPath, source,
...
fb.core.view.filter.LimitedFilter.prototype.updateFullNode = function(oldSnap, newSnap, optChangeAccumulator) {
  ...
  if (newSnap.isLeafNode() || newSnap.isEmpty()) {
    ...
    if (this.limit_ * 2 < newSnap.numChildren() && newSnap.isIndexed(this.index_)) {
      ...
      var iterator;
      newSnap = /** @type {!fb.core.snap.ChildrenNode} */ (newSnap);
      if (this.reverse_) {
      ...
      filtered = /** @type {!fb.core.snap.ChildrenNode} */ (filtered.updatePriority(fb.core.snap.EMPTY_NODE));
      var startPost;
      var endPost;
      var cmp;
      if (this.reverse_) {
 ...
 * @param {!fb.core.snap.Node} childSnap
 ...
 * @private
 */
fb.core.view.filter.LimitedFilter.prototype.fullLimitUpdateChild_ = function(snap, childKey, childSnap, source,
                                                                            optChangeAccumulator) {
  ...
  var oldEventCache = /** @type {!fb.core.snap.ChildrenNode} */ (snap);
  ...
  var windowBoundary = /** @type {!fb.core.snap.NamedNode} */
