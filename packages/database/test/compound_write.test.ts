...
describe('CompoundWrite Tests', () => {
  ...
  function assertNodeGetsCorrectPriority(compoundWrite, node, priority) {
    if (node.isEmpty()) {
      expect(apply(compoundWrite, node)).to.equal(EMPTY_NODE);
    } else {
      expect(apply(compoundWrite, node)).to.deep.equal(
  ...
  it('Empty merge is empty', () => {
    expect(isEmpty(CompoundWrite.empty())).to.be.true;
  });

  it('CompoundWrite with priority update is not empty.', () => {
    expect(
      isEmpty(addWrite(CompoundWrite.empty(), new Path('.priority'), PRIO_NODE))
    ).to.be.false;
  });

  it('CompoundWrite with update is not empty.', () => {
    expect(
      isEmpty(addWrite(CompoundWrite.empty(), new Path('foo/bar'), LEAF_NODE))
    ).to.be.false;
  });

  it('CompoundWrite with root update is not empty.', () => {
    expect(isEmpty(addWrite(CompoundWrite.empty(), Path.Empty, LEAF_NODE))).to
      .be.false;
  });

  it('CompoundWrite with empty root update is not empty.', () => {
    expect(isEmpty(addWrite(CompoundWrite.empty(), Path.Empty, EMPTY_NODE))).to
      .be.false;
  });

  it('CompoundWrite with root priority update, child write is not empty.', () => {
    const compoundWrite = addWrite(
      CompoundWrite.empty(),
      new Path('.priority'),
      PRIO_NODE
    );
    expect(isEmpty(childCompoundWrite(compoundWrite, new Path('.priority')))).to
      .be.false;
  });

  it('Applies leaf overwrite', () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, Path.Empty, LEAF_NODE);
    expect(apply(compoundWrite, EMPTY_NODE)).to.equal(LEAF_NODE);
  });

  it('Applies children overwrite', () => {
    let compoundWrite = CompoundWrite.empty();
    const childNode = EMPTY_NODE.updateImmediateChild('child', LEAF_NODE);
    compoundWrite = addWrite(compoundWrite, Path.Empty, childNode);
    expect(apply(compoundWrite, EMPTY_NODE)).to.equal(childNode);
  });

  it('Adds child node', () => {
    let compoundWrite = CompoundWrite.empty();
    const expected = EMPTY_NODE.updateImmediateChild('child', LEAF_NODE);
    compoundWrite = addWrite(compoundWrite, new Path('child'), LEAF_NODE);
    assertNodesEqual(expected, apply(compoundWrite, EMPTY_NODE));
  });
  ...
  it('Adds deep child node', () => {
    ...
    compoundWrite = addWrite(compoundWrite, path, LEAF_NODE);
    expect(apply(compoundWrite, EMPTY_NODE)).to.deep.equal(expected);
  });
  ...
  it('shallow update removes deep update', () => {
    ...
    compoundWrite = addWrite(compoundWrite, new Path('child-1/foo'), updateOne);
    compoundWrite = addWrite(compoundWrite, new Path('child-1/baz'), updateTwo);
    compoundWrite = addWrite(compoundWrite, new Path('child-1'), updateThree);
    ...
    assertNodesEqual(expected, apply(compoundWrite, CHILDREN_NODE));
  });
  ...
  it('child priority updates empty priority on child write', () => {
    ...
    assertNodeGetsCorrectPriority(
      childCompoundWrite(compoundWrite, new Path('child-1')),
      node,
      EMPTY_NODE
    );
  });
  ...
  it('deep priority set works on empty node when other set is available', () => {
    ...
    compoundWrite = addWrite(compoundWrite, new Path('foo/child'), LEAF_NODE);
    const node = apply(compoundWrite, EMPTY_NODE);
  ...
  it('child merge looks into update node', () => {
    ...
    compoundWrite = addWrite(compoundWrite, Path.Empty, update);
    assertNodesEqual(
      nodeFromJSON('foo-value'),
      apply(childCompoundWrite(compoundWrite, new Path('foo')), EMPTY_NODE)
    );
  });
  ...
  it('child merge removes node on deeper paths', () => {
    ...
    assertNodesEqual(
      EMPTY_NODE,
      apply(
        childCompoundWrite(compoundWrite, new Path('foo/not/existing')),
        LEAF_NODE
      )
    );
  });
  ...
  it('child merge with empty path is same merge', () => {
    ...
    const update = nodeFromJSON({ foo: 'foo-value', bar: 'bar-value' });
    compoundWrite = addWrite(compoundWrite, Path.Empty, update);
    expect(childCompoundWrite(compoundWrite, Path.Empty)).to.equal(
      compoundWrite
    );
  });
  ...
  it('root update removes root priority', () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, new Path('.priority'), PRIO_NODE);
    compoundWrite = addWrite(compoundWrite, Path.Empty, nodeFromJSON('foo'));
    assertNodesEqual(nodeFromJSON('foo'), apply(compoundWrite, EMPTY_NODE));
  });
  ...
  it('adding updates at path works', () => {
    ...
    compoundWrite = addWrites(compoundWrite, new Path('child-1'), updates);
  ...
  it('adding updates at root works', () => {
    ...
    compoundWrite = addWrites(compoundWrite, Path.Empty, updates);
    ...
    assertNodesEqual(
      ...
      apply(compoundWrite, CHILDREN_NODE)
    );
  });
  ...
  it('child write of root priority works', () => {
    ...
    assertNodesEqual(
      PRIO_NODE,
      apply(
        childCompoundWrite(compoundWrite, new Path('.priority')),
        EMPTY_NODE
      )
    );
  });
  ...
  it('complete children only returns complete overwrites', () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, new Path('child-1'), LEAF_NODE);
    expect(getCompleteChildren(compoundWrite)).to.deep.equal([
  ...
  it('complete children only returns empty overwrites', () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, new Path('child-1'), EMPTY_NODE);
    expect(getCompleteChildren(compoundWrite)).to.deep.equal([
  ...
  it('complete children return all complete children but no incomplete', () => {
    ...
    compoundWrite = addWrite(compoundWrite, new Path('child-2'), LEAF_NODE);
    compoundWrite = addWrite(compoundWrite, new Path('child-3'), EMPTY_NODE);
    const expected = {
      'child-2': LEAF_NODE,
      'child-3': EMPTY_NODE
    };
    const actual = {};
    const completeChildren = getCompleteChildren(compoundWrite);
  ...
  it('complete children return all children for root set', () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, Path.Empty, CHILDREN_NODE);
  ...
  it('empty merge has no shadowing write', () => {
    expect(hasCompleteWrite(CompoundWrite.empty(), Path.Empty)).to.be.false;
  });
  ...
  it('compound write with empty root has shadowing write', () => {
    ...
    expect(hasCompleteWrite(compoundWrite, Path.Empty)).to.be.true;
    expect(hasCompleteWrite(compoundWrite, new Path('child'))).to.be.true;
  });
  ...
  it('compound write with deep update has shadowing write', () => {
    ...
    expect(hasCompleteWrite(compoundWrite, Path.Empty)).to.be.false;
    expect(hasCompleteWrite(compoundWrite, new Path('deep'))).to.be.false;
    expect(hasCompleteWrite(compoundWrite, new Path('deep/update'))).to.be.true;
  });
  ...
  it('compound write with priority update has shadowing write', () => {
    ...
    expect(hasCompleteWrite(compoundWrite, Path.Empty)).to.be.false;
    expect(hasCompleteWrite(compoundWrite, new Path('.priority'))).to.be.true;
  });

  it('updates can be removed', () => {
    let compoundWrite = CompoundWrite.empty();
    const update = nodeFromJSON({ foo: 'foo-value', bar: 'bar-value' });
    compoundWrite = addWrite(compoundWrite, new Path('child-1'), update);
    compoundWrite = removeWrite(compoundWrite, new Path('child-1'));
    assertNodesEqual(CHILDREN_NODE, apply(compoundWrite, CHILDREN_NODE));
  });
  ...
  it('deep removes has no effect on overlaying set', () => {
    ...
    compoundWrite = addWrite(compoundWrite, new Path('child-1'), updateOne);
    compoundWrite = addWrite(compoundWrite, new Path('child-1/baz'), updateTwo);
    compoundWrite = addWrite(
      compoundWrite,
    ...
    compoundWrite = removeWrite(compoundWrite, new Path('child-1/foo'));
    const expectedChildOne = {
      foo: 'new-foo-value',
      bar: 'bar-value',
      baz: 'baz-value'
  ...
  it('remove at path without set is without effect', () => {
    ...
    compoundWrite = removeWrite(compoundWrite, new Path('child-2'));
    const expectedChildOne = {
      foo: 'new-foo-value',
      bar: 'bar-value',
      baz: 'baz-value'
  ...
  it('can remove priority', () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, new Path('.priority'), PRIO_NODE);
    compoundWrite = removeWrite(compoundWrite, new Path('.priority'));
  ...
  it('remove at root also removes priority', () => {
    ...
    compoundWrite = removeWrite(compoundWrite, Path.Empty);
  ...
  it('updating priority doesnt overwrite leaf node', () => {
    ...
    assertNodesEqual(LEAF_NODE, apply(compoundWrite, EMPTY_NODE));
  });

  it("updating empty node doesn't overwrite leaf node", () => {
    let compoundWrite = CompoundWrite.empty();
    compoundWrite = addWrite(compoundWrite, Path.Empty, LEAF_NODE);
    compoundWrite = addWrite(compoundWrite, new Path('child'), EMPTY_NODE);
    assertNodesEqual(LEAF_NODE, apply(compoundWrite, EMPTY_NODE));
  });
  ...
  it('Overwrites existing child', () => {
    ...
    compoundWrite = addWrite(compoundWrite, path, LEAF_NODE);
    expect(apply(compoundWrite, CHILDREN_NODE)).to.deep.equal(
  ...
  it('Updates priority of child', () => {
    ...
    compoundWrite = addWrite(compoundWrite, path, PRIO_NODE);
    assertNodesEqual(
