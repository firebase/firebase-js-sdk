...
export const firebaseApp = firebase as FirebaseApp;
 ...
 */
export function createPolyfill(deps?: Dependencies): () => FirebaseAuth {
  return function() {
    if (memo) {
      return memo;
    }
    const auth: Auth = initializeAuth(firebase.app(), deps);
