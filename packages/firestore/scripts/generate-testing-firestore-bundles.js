...
class TestBundleBuilder {
    ...
    addDocumentMetadata(docKey, readTime, exists) {
        this.elements.push({
            documentMetadata: {
    ...
    addDocument(docKey, createTime, updateTime, fields) {
        this.elements.push({
            document: {
                name: toName(this.serializer, docKey),
                createTime,
    ...
    addNamedQuery(name, readTime, query) {
        let bundledLimitType = !!query.limit
            ? 'FIRST'
            : undefined;
        if (query.limitType === "L" /* Last */) {
            query = queryWithLimit(query, query.limit, "F" /* First */);
    ...
    getMetadataElement(id, createTime, version = 1) {
    ...
    build(id, createTime, version = 1) {
        let result = '';
        for (const element of this.elements) {
            result += lengthPrefixedString(element);
        }
        return (lengthPrefixedString(this.getMetadataElement(id, createTime, version)) +
            result);
    }
}
 ...
 * limitations under the License.
 */
const LOG_TAG$1 = 'SharedClientState';
/**
...
class MutationMetadata {
    ...
    static fromWebStorageEntry(user, batchId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$1, `Failed to parse mutation state for ID '${batchId}': ${value}`);
            return null;
        }
    }
    toWebStorageJSON() {
...
class QueryTargetMetadata {
    ...
    static fromWebStorageEntry(targetId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$1, `Failed to parse target state for ID '${targetId}': ${value}`);
            return null;
        }
    }
    toWebStorageJSON() {
...
class RemoteClientState {
    ...
    static fromWebStorageEntry(clientId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$1, `Failed to parse client data for instance '${clientId}': ${value}`);
            return null;
        }
    }
}
...
class SharedOnlineState {
    ...
    static fromWebStorageEntry(value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$1, `Failed to parse online state: ${value}`);
            return null;
        }
    }
}
...
class WebStorageSharedClientState {
    ...
    getItem(key) {
        ...
        logDebug(LOG_TAG$1, 'READ', key, value);
        return value;
    }
    setItem(key, value) {
        logDebug(LOG_TAG$1, 'SET', key, value);
        this.storage.setItem(key, value);
    }
    removeItem(key) {
        logDebug(LOG_TAG$1, 'REMOVE', key);
    ...
    handleWebStorageEvent(event) {
        ...
        if (storageEvent.storageArea === this.storage) {
            logDebug(LOG_TAG$1, 'EVENT', storageEvent.key, storageEvent.newValue);
    ...
    async handleMutationBatchEvent(mutationBatch) {
        if (mutationBatch.user.uid !== this.currentUser.uid) {
            logDebug(LOG_TAG$1, `Ignoring mutation for non-active user ${mutationBatch.user.uid}`);
            return;
        }
...
function fromWebStorageSequenceNumber(seqString) {
    ...
    if (seqString != null) {
        try {
            ...
            logError(LOG_TAG$1, 'Failed to read sequence number from WebStorage', e);
        }
    }
 ...
 */
function loadProtos() {
    const root = path$1.resolve(__dirname, process.env.FIRESTORE_PROTO_ROOT || '../../protos');
    const firestoreProtoFile = path$1.join(root, 'google/firestore/v1/firestore.proto');
...
class FirestoreSettings {
    constructor(settings) {
        ...
        this.ignoreUndefinedProperties = (_b = settings.ignoreUndefinedProperties) !== null && _b !== void 0 ? _b : DEFAULT_IGNORE_UNDEFINED_PROPERTIES;
        ...
        this.experimentalForceLongPolling = (_c = settings.experimentalForceLongPolling) !== null && _c !== void 0 ? _c : DEFAULT_FORCE_LONG_POLLING;
        validateNamedOptionalType('settings', 'boolean', 'experimentalAutoDetectLongPolling', settings.experimentalAutoDetectLongPolling);
        this.experimentalAutoDetectLongPolling = (_d = settings.experimentalAutoDetectLongPolling) !== null && _d !== void 0 ? _d : DEFAULT_FORCE_AUTO_DETECT_LONG_POLLING;
...
class Firestore {
    ...
    enablePersistence(settings) {
        ...
        if (settings) {
            synchronizeTabs = (_a = settings.synchronizeTabs) !== null && _a !== void 0 ? _a : DEFAULT_SYNCHRONIZE_TABS;
            experimentalForceOwningTab = (_b = settings.experimentalForceOwningTab) !== null && _b !== void 0 ? _b : DEFAULT_FORCE_OWNING_TAB;
...
const TEST_DATABASE_ID = new DatabaseId(TEST_PROJECT);
const TEST_PERSISTENCE_KEY = '[PersistenceTestHelpers]';
/** The persistence prefix used for testing in IndexedBD and LocalStorage. */
const TEST_PERSISTENCE_PREFIX = indexedDbStoragePrefix(TEST_DATABASE_ID, TEST_PERSISTENCE_KEY);
/**
 * The database name used by tests that access IndexedDb. To be used in
 * conjunction with `TEST_DATABASE_INFO` and
 * `TEST_DATABASE_ID`.
 */
const INDEXEDDB_TEST_DATABASE_NAME = indexedDbStoragePrefix(TEST_DATABASE_ID, TEST_PERSISTENCE_KEY) +
    MAIN_DATABASE;
...
const FIRESTORE = new Firestore({
    ...
    database: '(default)'
}, new component.Provider('auth-internal', new component.ComponentContainer('default')), new IndexedDbPersistenceProvider());
function firestore() {
    return FIRESTORE;
}
function collectionReference(path$1) {
    const firestoreClient = firestore();
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
    firestoreClient.ensureClientConfigured();
    return new CollectionReference(path(path$1), firestoreClient, 
    /* converter= */ null);
}
...
function path(path, offset) {
    return new ResourcePath(splitPath(path, '/'), offset);
}
function splitPath(path, splitChar) {
    if (path === '') {
        return [];
    }
    else {
...
const ALT_EMULATOR_PROJECT_ID = 'test-db2';
 ...
 */
function bundleWithTestDocsAndQueries(projectId) {
    const testDocs = {
    ...
    builder.addNamedQuery('limit', { seconds: 1000, nanos: 9999 }, collectionReference('coll-1')
        .orderBy('bar', 'desc')
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .limit(1)._query);
    builder.addNamedQuery('limit-to-last', { seconds: 1000, nanos: 9999 }, collectionReference('coll-1')
        .orderBy('bar', 'desc')
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .limitToLast(1)._query);
    builder.addDocumentMetadata(docA, { seconds: 1000, nanos: 9999 }, true);
    builder.addDocument(docA, { seconds: 1, nanos: 9 }, { seconds: 1, nanos: 9 }, testDocs.a);
    builder.addDocumentMetadata(docB, { seconds: 1000, nanos: 9999 }, true);
    builder.addDocument(docB, { seconds: 1, nanos: 9 }, { seconds: 1, nanos: 9 }, testDocs.b);
    return builder;
}
const PROJECT_IDS = [
...
const argv = yargs.options({
    projectIds: {
        ...
        type: 'string',
        demandOption: false,
        desc: 'Project IDs to use to generate testing bundle files. A set of default project IDs will be used if not specified.'
    },
}).argv;
function generate(ids) {
    ...
    let projectIds = ids !== null && ids !== void 0 ? ids : PROJECT_IDS;
    const result = {};
    for (const projecId of projectIds) {
        result[projecId] = bundleWithTestDocsAndQueries(projecId).build('test-bundle', { seconds: 1001, nanos: 9999 });
    }
