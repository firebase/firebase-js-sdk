...
apiDescribe('Aggregation queries', (persistence: boolean) => {
  ...
  it('rejects bad aliases when using getAggregationFromServer unquoted', () => {
    const testDocs = {
      a: { author: 'authorA', title: 'titleA' },
      b: { author: 'authorB', title: 'titleB' }
    };
  ...
  it('allows special chars in quoted aliases when using getAggregationFromServer', () => {
    const testDocs = {
      a: { author: 'authorA', title: 'titleA' },
      b: { author: 'authorB', title: 'titleB' }
    };
    return withTestCollection(persistence, testDocs, async coll => {
      ...
      // Note that the backticks (quotes) are removed from the alias
      // in the returned object
      expect(snapshot.data()['with-un/su+pp[or]ted']).to.equal(2);
    });
  });

  it('rejects bad quoted aliases when using getAggregationFromServer', () => {
    const testDocs = {
      a: { author: 'authorA', title: 'titleA' },
      b: { author: 'authorB', title: 'titleB' }
    };
