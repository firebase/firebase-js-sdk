...
(USE_EMULATOR ? apiDescribe.skipClassic : apiDescribe.skip)(
  'RealtimePipelines',
  persistence => {
    addEqualityMatcher();
    ...
    it('basic listen', async () => {
      ...
      expect(snapshot.results[2].data()!['title']).toBe(
      ...
      expect(snapshot.results[1].data()!['title']).toBe(
        'The Lord of the Rings'
      );
    ...
    it('can evaluate server timestamp with "estimate" option', async () => {
      ...
      const unsubscribe = _onRealtimePipelineSnapshot(
        firestore
          ...
          .collection(randomCol.path)
          .where(
            field('rating').timestampAdd('microsecond', 1).greaterThan(now)
          ),
    ...
    it('same pipeline with different options', async () => {
      const storeEvent1 = new PipelineEventsAccumulator();
      const storeEvent2 = new PipelineEventsAccumulator();
      await disableNetwork(firestore);
