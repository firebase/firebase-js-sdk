      ...
      () => {
        it('can raise snapshot from cache for Query', () => {
          ...
          return withTestCollection(persistence, testDocs, async coll => {
            await getDocs(pipelineMode, coll); // Populate the cache.
          ...
          return withTestDocAndInitialData(
            ...
            async docRef => {
              ...
              if (pipelineMode === 'query-to-pipeline') {
                ...
                const storeEvent =
                  new PipelineEventsAccumulator<RealtimePipelineSnapshot>();
                const unsubscribe = onSnapshot(
                  pipelineMode,
                ...
                const storeEvent =
                  new PipelineEventsAccumulator<DocumentSnapshot>();
        ...
        it('will not be triggered by transactions while listening to cache', () => {
          return withTestCollection(persistence, {}, async (coll, db) => {
            const accumulator = new PipelineEventsAccumulator<QuerySnapshot>();
            const unsubscribe = onSnapshot(
              pipelineMode,
              coll,
              { source: 'cache' },
