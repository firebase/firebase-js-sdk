...
abstract class TestRunner {
  ...
  /**
   * Maps SDK target IDs (assigned by the spec builder based on the query) to
   * the list of remote target IDs generated by `RemoteStore`.
   *
   * In spec tests, the spec builder reuses the same SDK ID if the user unlistens
   * and listens again to the same query. However, `RemoteStore` generates a
   * brand new, unique remote ID every time a new listen is opened on the stream.
   * To track these correctly, we map the SDK ID to a list of remote IDs and use
  ...
  private validateExpectedSnapshotEvents(
    ...
    if (expectedEvents) {
      ...
      let expectedEventsSorted: SnapshotEvent[] = [];
      if (this.convertToPipeline) {
        expectedEventsSorted = expectedEvents.sort((a, b) =>
          primitiveComparator(
            canonifyQueryOrPipeline(
              toCorePipeline(
                pipelineFromStages(
                  toPipelineStages(
                    parseQuery(a.query as SpecQuery),
                    newTestFirestore()
                  )
                )
              )
            ...
            canonifyQueryOrPipeline(
              toCorePipeline(
                pipelineFromStages(
                  toPipelineStages(
                    parseQuery(b.query as SpecQuery),
                    newTestFirestore()
                  )
                )
              )
        ...
        expectedEventsSorted = expectedEvents.sort((a, b) =>
          primitiveComparator(
            canonifyQueryOrPipeline(parseQuery(a.query as SpecQuery)),
            canonifyQueryOrPipeline(parseQuery(b.query as SpecQuery))
          )
        );
      }
  ...
  private async validateActiveTargets(): Promise<void> {
    ...
    this.expectedActiveTargets.forEach((expected, sdkTargetId) => {
      ...
      if (
        ...
        const actualPipeline = fromPipelineTarget(
          actualTarget.pipelineQuery!,
          this.serializer
        );
        ...
        if (targetIsPipelineTarget(targetData.target)) {
          ...
          const expectedQuery = new QueryImpl(
            ...
            targetData.target.collectionGroup,
            targetData.target.orderBy,
            targetData.target.filters,
            targetData.target.limit,
            LimitType.First,
            targetData.target.startAt,
            targetData.target.endAt
          );
          const expectedPipeline = toCorePipeline(
            pipelineFromStages(
    ...
    if (this.convertToPipeline) {
      expect(
        pipelineEq(
          toCorePipeline(
            pipelineFromStages(
              toPipelineStages(parseQuery(expected.query), newTestFirestore())
            )
          ),
          actual.query as CorePipeline
        )
      );
    } else {
