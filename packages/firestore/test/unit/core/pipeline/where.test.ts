...
describe('Where Stage', () => {
  ...
  it('duplicateConditions', () => {
    ...
    const pipeline = db
      ...
      .where(
        and(
          field('age').greaterThanOrEqual(constant(10)),
          field('age').greaterThanOrEqual(constant(20))
        )
      );
  ...
  it('logicalEquivalentCondition_equal', () => {
    ...
    const pipeline1 = db
      .pipeline()
      .database()
      .where(field('age').equal(constant(25)));
    const pipeline2 = db
      .pipeline()
      .database()
      .where(constant(25).equal(field('age')));
  ...
  it('logicalEquivalentCondition_and', () => {
    ...
    const pipeline1 = db
      ...
      .where(
        and(
          field('age').greaterThan(constant(10)),
          field('age').lessThan(constant(70))
        )
      );
    const pipeline2 = db
      .pipeline()
  ...
  it('logicalEquivalentCondition_or', () => {
    ...
    const pipeline1 = db
      ...
      .where(
        or(
          field('age').lessThan(constant(10)),
          field('age').greaterThan(constant(80))
        )
      );
    const pipeline2 = db
      .pipeline()
  ...
  it('andExpression_logicallyEquivalent_toSeparatedStages', () => {
    ...
    const equalityArgument1 = field('a').equal(constant(1));
    const equalityArgument2 = field('b').equal(constant(2));
