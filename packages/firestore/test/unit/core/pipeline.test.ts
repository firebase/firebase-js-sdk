...
function and(expr1: Expr, expr2: Expr): BooleanExpr {
  return apiAnd(expr1 as BooleanExpr, expr2 as BooleanExpr);
}

function or(expr1: Expr, expr2: Expr): BooleanExpr {
  return apiOr(expr1 as BooleanExpr, expr2 as BooleanExpr);
}

function not(expr: Expr): BooleanExpr {
  return apiNot(expr as BooleanExpr);
}

function xor(expr1: Expr, expr2: Expr): BooleanExpr {
...
describe('runPipeline()', () => {
  ...
  describe('Complex Queries', () => {
    ...
    it('where_withLargeNumberOrs', () => {
      ...
      const pipeline = db
        ...
        .collection(`/${COLLECTION_ID}`)
        .where(
          apiOr(orConditions[0], orConditions[1], ...orConditions.slice(2))
        );
  ...
  describe('Null Semantics', () => {
    ...
    it('where_eq_map_withNullArray', () => {
      ...
      expect(runPipeline(pipeline, [doc1, doc2, doc3, doc4, doc5, doc6, doc7]))
        .to.be.empty;
    });
    ...
    it('where_arrayContainsAll_partialNull', () => {
      ...
      const pipeline = db
        ...
        .collection('/users')
        .where(
          arrayContainsAll(field('score'), [constant(null), constant(42)])
        );
