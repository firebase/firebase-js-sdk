...
export const falseExpr = constant(1).equal(2);
export const trueExpr = constant(1).equal(1);

export function isTypeComparable(left: Constant, right: Constant): boolean {
  const reader = newUserDataReader(db);
  left._readUserData(
    reader.createContext(UserDataSource.Argument, 'isTypeComparable')
  );
  right._readUserData(
    reader.createContext(UserDataSource.Argument, 'isTypeComparable')
  );
...
export class ComparisonValueTestData {
  ...
  static equivalentValues(): Array<{ left: Expression; right: Expression }> {
  ...
  static lessThanValues(): Array<{ left: Expression; right: Expression }> {
    const results: Array<{ left: Expression; right: Expression }> = [];
    ...
    for (
      ...
      for (
        ...
        if (isTypeComparable(left as Constant, right as Constant)) {
  ...
  static greaterThanValues(): Array<{ left: Expression; right: Expression }> {
    const results: Array<{ left: Expression; right: Expression }> = [];
    ...
    for (
      ...
      for (
        ...
        const right =
          ComparisonValueTestData.ALL_SUPPORTED_COMPARABLE_VALUES[j];
        if (isTypeComparable(right as Constant, left as Constant)) {
  ...
  static mixedTypeValues(): Array<{ left: Expression; right: Expression }> {
    const results: Array<{ left: Expression; right: Expression }> = [];
...
export function evaluateToValue(
  ...
  const reader = newUserDataReader(db);
  expr._readUserData(
    reader.createContext(UserDataSource.Argument, 'evaluateToValue')
  );
...
export function evaluateToResult(
  ...
  const reader = newUserDataReader(db);
  expr._readUserData(
    reader.createContext(UserDataSource.Argument, 'evaluateToResult')
  );
  return toEvaluable(expr).evaluate(
    // @ts-ignore
    { serializer: reader.serializer },
...
export function errorExpr(): Expression {
  return field('not-an-array').arrayLength();
}

export function errorFilterCondition(): BooleanExpression {
  return field('not-an-array').greaterThan(0);
}

export function expectEqualToConstant(
  evaluated: Value,
