...
describe('Debugging Functions', () => {
  describe('exists', () => {
    ...
    it('null_returnsTrue', () => {
      expect(evaluateToValue(exists(constant(null)))).toEqual(TRUE_VALUE);
    });

    it('error_returnsError', () => {
      expect(evaluateToValue(exists(errorExpr()))).toBeUndefined();
    });

    it('unset_withNotExists_returnsTrue', () => {
      const functionExpr = exists(UNSET_VALUE);
      expect(evaluateToValue(not(functionExpr))).toEqual(TRUE_VALUE);
    });

    it('unset_returnsFalse', () => {
      expect(evaluateToValue(exists(UNSET_VALUE))).toEqual(FALSE_VALUE);
    });

    it('emptyArray_returnsTrue', () => {
      expect(evaluateToValue(exists(constantArray([])))).toEqual(TRUE_VALUE);
    });

    it('emptyMap_returnsTrue', () => {
      expect(evaluateToValue(exists(constantMap({})))).toEqual(TRUE_VALUE);
    });
  });
  ...
  describe('isError', () => {
    it('error_returnsTrue', () => {
      ...
      expect(evaluateToValue(isError(errorExpr()))).toEqual(TRUE_VALUE);
    });
    ...
    it('non_error_returnsFalse', () => {
      expect(evaluateToValue(isError(constant(42)))).toEqual(FALSE_VALUE);
    });

    it('explicit_null_returnsFalse', () => {
      // constant(null) evaluates to the NullValue proto
      expect(evaluateToValue(isError(constant(null)))).toEqual(FALSE_VALUE);
    });

    it('unset_returnsFalse', () => {
      // UNSET_VALUE represents reading a non-existent field, which is not an error itself.
      expect(evaluateToValue(isError(UNSET_VALUE))).toEqual(FALSE_VALUE);
    });
    ...
    it('anythingButError_returnsFalse', () => {
      ...
      expect(evaluateToValue(isError(constant(null)))).toEqual(FALSE_VALUE);
      expect(evaluateToValue(isError(constant(0)))).toEqual(FALSE_VALUE);
    });
  });
});
