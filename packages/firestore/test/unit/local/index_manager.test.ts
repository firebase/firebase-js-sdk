...
describe('index_manager.ts top-level functions', () => {
  describe('displayNameForIndexType()', () => {
    it('IndexType.NONE', () =>
      expect(displayNameForIndexType(IndexType.NONE)).toBe('NONE'));

    it('IndexType.FULL', () =>
      expect(displayNameForIndexType(IndexType.FULL)).toBe('FULL'));

    it('IndexType.PARTIAL', () =>
      expect(displayNameForIndexType(IndexType.PARTIAL)).toBe('PARTIAL'));
    ...
    it('invalid IndexType', () =>
      ...
      expect(displayNameForIndexType('zzyzx')).toBe(
...
describe.skipIf(!IndexedDbPersistence.isAvailable())(
  ...
  async () => {
    ...
    async function getIndexManager(
      ...
      return new TestIndexManager(
        persistence,
        persistence.getIndexManager(user)
      );
    }
    ...
    it('can add indexes', async () => {
      ...
      expect(fieldIndexes[0]).toEqual(fieldIndex('coll1', { id: 1 }));
    });
    ...
    it('uses auto-incrementing index id', async () => {
      ...
      expect(fieldIndexes[0]).toEqual(fieldIndex('coll1', { id: 1 }));
      expect(fieldIndexes[1]).toEqual(fieldIndex('coll2', { id: 2 }));
    });

    it('can get indexes', async () => {
      let fieldIndexes = await indexManager.getFieldIndexes('coll1');
      expect(fieldIndexes).toHaveLength(0);
    ...
    it('can update collection group', async () => {
      ...
      expect(fieldIndexes[0].indexState).toEqual(
        new IndexState(/* sequenceNumber= */ 0, IndexOffset.min())
      );
      expect(fieldIndexes[1].indexState).toEqual(
        new IndexState(/* sequenceNumber= */ 0, IndexOffset.min())
      );
      expect(fieldIndexes[2].indexState).toEqual(
        new IndexState(/* sequenceNumber= */ 0, IndexOffset.min())
      );
    ...
    it('deleting field index removes entry from getNextCollectionGroupToUpdate()', async () => {
      await indexManager.addFieldIndex(fieldIndex('coll1'));
      expect(await indexManager.getNextCollectionGroupToUpdate()).toBe('coll1');

      const actualIndex = (await indexManager.getFieldIndexes())[0];
      await indexManager.deleteFieldIndex(actualIndex);

      expect(await indexManager.getNextCollectionGroupToUpdate()).toBe(null);
    });
    ...
    it('persist index offset', async () => {
      ...
      expect(indexes[0].indexState.offset).toEqual(offset);
    });
    ...
    it('returns empty result when no index exists', async () => {
      ...
      expect(await indexManager.getIndexType(queryToTarget(q))).toBe(
        IndexType.NONE
      );
      expect(
        await indexManager.getDocumentsMatchingTarget(queryToTarget(q))
      ).toBeNull();
    });
    ...
    it('createTargetIndexes() creates full indexes for each sub-target', async () => {
      ...
      const subQuery1 = queryWithAddedFilter(
        query('coll'),
        filter('a', '==', 1)
      );
      const subQuery2 = queryWithAddedFilter(
        query('coll'),
        filter('b', '==', 2)
      );
      const subQuery3 = queryWithAddedFilter(
        query('coll'),
        filter('c', '==', 3)
      );
      ...
      it('can index BSON Binary Data fields', async () => {
        ...
        q = queryWithAddedFilter(
          ...
          filter(
            'key',
            '>=',
            Bytes.fromUint8Array(new Uint8Array([1, 2, 4]), 1)
          )
        );
        await verifyResults(q, 'coll/doc2', 'coll/doc3');
...
function genericIndexManagerTests(
  ...
  it('can add and read collection=>parent index entries', async () => {
    ...
    expect(await indexManager.getCollectionParents('messages')).toEqual([
      path(''),
      path('rooms/bar'),
      path('rooms/foo')
    ]);
    expect(await indexManager.getCollectionParents('messages2')).toEqual([
      path('rooms/foo')
    ]);
    expect(await indexManager.getCollectionParents('messages3')).toEqual([]);
  });
}
