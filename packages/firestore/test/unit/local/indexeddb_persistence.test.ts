...
describe.only('all indexeddb', () => {
  ...
  describe('IndexedDbSchema: createOrUpgradeDb', () => {
    ...
    it('drops the query cache from 2 to 3', () => {
      ...
      const expectedMutation = new DbMutationBatch(
        userId,
        batchId,
        1000,
        [],
    ...
    it('can upgrade from schema version 4 to 5', () => {
      ...
      const testWritePending = {
        update: {
          name:
            'projects/test-project/databases/(default)/documents/docs/pending'
        }
      };
      const testMutations: DbMutationBatch[] = [
      ...
      await withDb(9, db => {
        ...
        return sdb.runTransaction('readwrite', V8_STORES, txn => {
          ...
          return targetsStore.put(
            TEST_SERIALIZER.toDbTarget(initialTargetData)
          );
        });
      });
