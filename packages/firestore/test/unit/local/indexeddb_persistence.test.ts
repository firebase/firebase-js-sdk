...
describe.skipIf(!IndexedDbPersistence.isAvailable())(
  ...
  () => {
    ...
    afterAll(() => SimpleDb.delete(INDEXEDDB_TEST_DATABASE_NAME));
    ...
    function verifyUserHasDocumentOverlay(
      ...
      return documentOverlayStore.get(key).next(overlay => {
        ...
        expect(overlay!.overlayMutation).toEqual(expected);
      });
    }
              ...
              return targets
                ...
                .next(targetGlobalEntry => {
                  ...
                  const expected = JSON.parse(
                    JSON.stringify(resetTargetGlobal)
                  );
                  expect(targetGlobalEntry).toEqual(expected);
                })
                .next(() => mutations.get(batchId))
                .next(mutation => {
                  // Mutations should be unaffected.
                  expect(mutation!.userId).toBe(userId);
                  expect(mutation!.batchId).toBe(batchId);
                });
            }
          );
        });
              ...
              let p = PersistencePromise.forEach(
                ...
                (testMutation: DbMutationBatch) =>
                  store.get(testMutation.batchId).next(mutationBatch => {
                    expect(mutationBatch).toEqual(testMutation);
                  })
              );
              p = p.next(() => {
                store
          ...
          txn => {
            ...
            return targetDocumentStore.iterate(
              ...
              ([_, path], targetDocument) => {
                ...
                expect(targetDocument.sequenceNumber).toBe(expected);
              }
            );
          }
        );
        ...
        return db.runTransaction(
          ...
          txn => {
            ...
            return collectionParentsStore
              ...
              .next(() => {
                expect(actualParents).toEqual(expectedParents);
              });
          }
        );
      });
    ...
    it('rewrites canonical IDs during upgrade from version 9 to 10', async () => {
      await withDb(9, db => {
        return db.runTransaction(
          ...
          V8_STORES,
          txn => {
            const targetsStore = txn.store<DbTargetKey, DbTarget>(
              DbTargetStore
            );
            ...
            const serializedData = toDbTarget(
              TEST_SERIALIZER,
              initialTargetData
            );
        ...
        return db.runTransaction(
          ...
          txn => {
            ...
            return targetsStore.iterate((key, value) => {
              ...
              expect(actualCanonicalId).toBe(expectedCanonicalId);
            });
          }
        );
      });
    ...
    it('can upgrade from schema version 13 to 14 (overlay migration)', () => {
    ...
    it('downgrading throws a custom error', async () => {
      ...
      try {
        ...
        await db.ensureDb(expect.getState().currentTestName ?? 'test');
      } catch (e) {
  ...
  () => {
          ...
          async (thatPersistence, thatDocument, thatQueue) => {
            ...
            await withMultiClientPersistence(
              ...
              async (thisPersistence, thisDocument, thisQueue) => {
                ...
                expect(isPrimary).toBe(primaryState);
              }
            );
          }
        );
    ...
    it('regains lease if available', () => {
      return withPersistence('clientA', async persistence => {
        expect(await getCurrentLeaseOwner()).not.toBeNull();

        await clearPrimaryLease();
        expect(await getCurrentLeaseOwner()).toBeNull();
...
describe.skipIf(!IndexedDbPersistence.isAvailable())(
  'IndexedDb: allowTabSynchronization',
  () => {
    beforeEach(() => SimpleDb.delete(INDEXEDDB_TEST_DATABASE_NAME));
    ...
    it('blocks start() when getHighestListenSequenceNumber() fails', async () => {
      await withUnstartedCustomPersistence(
        ...
        async db1 => {
          ...
          await expect(db1.start()).rejects.toThrow(
...
describe.skipIf(!IndexedDbPersistence.isAvailable())('IndexedDb', () => {
