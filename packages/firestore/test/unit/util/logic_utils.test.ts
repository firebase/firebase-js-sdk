...
describe('LogicUtils', () => {
  ...
  it("implements field filter distribution over and filter", () => {
    expect(filterEquals(applyDistribution(andFilter(A, B, C), D), andFilter(A, B, C, D))).to.be.true;
  });

  it("implements field filter distribution over or filter", () => {
    // A & (B | C | D) = (A & B) | (A & C) | (A & D)
    // (B | C | D) & A = (A & B) | (A & C) | (A & D)
    const expected = orFilter(andFilter(A, B), andFilter(A, C), andFilter(A, D));
    expect(filterEquals(applyDistribution(A, orFilter(B, C, D)), expected)).to.be.true;
    expect(filterEquals(applyDistribution(orFilter(B, C, D), A), expected)).to.be.true;
  });
  ...
  it("implements and filter distribution with and filter", () => {
    ...
    expect(filterEquals(applyDistribution(andFilter(A, B), andFilter(C, D)), expectedResult)).to.be.true;
  });

  it("implements and filter distribution with or filter", () => {
      // (A & B) & (C | D) --> (A & B & C) | (A & B & D)
      const expectedResult = orFilter(andFilter(A, B, C), andFilter(A, B, D));
      expect(filterEquals(applyDistribution(andFilter(A, B), orFilter(C, D)), expectedResult)).to.be.true;
    }
  );
  ...
  it("implements or filter distribution with and filter", () => {
    ...
    expect(filterEquals(applyDistribution(orFilter(A, B), andFilter(C, D)), expectedResult)).to.be.true;
  });
  ...
  it("implements or filter distribution with or filter", () => {
    ...
    expect(filterEquals(applyDistribution(orFilter(A, B), orFilter(C, D)), expectedResult));
  });
  ...
  it("implements compute dnf flat AND filter", () => {
    ...
    expect(computeDistributedNormalForm(compositeFilter)).to.deep.equal(compositeFilter);
  ...
  it("compute DNF2", () => {
    ...
    expect(computeDistributedNormalForm(compositeFilter)).to.deep.equal(expectedResult);
  ...
  it("compute DNF6", () => {
    ...
    // -> (A & B) | (A & C & D) | (A & C & E & F)
    const compositeFilter =
      andFilter(A, orFilter(B, andFilter(C, orFilter(D, andFilter(E, F)))));
    const expectedDnfTerms = [andFilter(A, B), andFilter(C, D, A), andFilter(E, F, C, A)];
    const expectedResult = orFilter(...expectedDnfTerms);
    expect(computeDistributedNormalForm(compositeFilter)).to.deep.equal(expectedResult);
    expect(getDnfTerms(compositeFilter)).to.deep.equal(expectedDnfTerms);
  });
  ...
  it("compute DNF8", () => {
    ...
    const expectedDnfTerms = [
      andFilter(E, F, A, B), andFilter(G, H, A, B), andFilter(E, F, C, D), andFilter(G, H, C, D)
    ];
    const expectedResult = orFilter(...expectedDnfTerms);
    expect(computeDistributedNormalForm(compositeFilter)).to.deep.equal(expectedResult);
    expect(getDnfTerms(compositeFilter)).to.deep.equal(expectedDnfTerms);
