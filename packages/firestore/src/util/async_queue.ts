...
export class AsyncQueue {
  ...
  private delayedOperations: Array<DelayedOperation<AnyJs | void>> = [];
   ...
   */
  enqueue<T extends AnyJs | void>(op: () => Promise<T>): Promise<T> {
  ...
  enqueueAfterDelay<T extends (AnyJs | void)>(
    ...
    const delayedOp = DelayedOperation.createAndSchedule<AnyJs | void>(
      this,
      timerId,
      delayMs,
      op,
  ...
  private removeDelayedOperation<T>(op: DelayedOperation<AnyJs | void>): void {
