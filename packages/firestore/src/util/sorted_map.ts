...
export class LLRBNode<K, V> {
  ...
  protected check(): number {
    if (this.isRed() && this.left.isRed()) {
      throw fail(0xaad28107, 'Red node has red child', {
        key: this.key,
        value: this.value
      });
    }
    if (this.right.isRed()) {
      throw fail(0x37210df9, 'Right child of (`key`, `value`) is red', {
        key: this.key,
        value: this.value
      });
    }
    ...
    if (blackDepth !== (this.right as LLRBNode<K, V>).check()) {
      throw fail(0x6d2d0edf, 'Black depths differ');
    } else {
...
export class LLRBEmptyNode<K, V> {
  get key(): never {
    throw fail(0xe1a6d03e, 'LLRBEmptyNode has no key.');
  }
  get value(): never {
    throw fail(0x3f0d8793, 'LLRBEmptyNode has no value.');
  }
  get color(): never {
    throw fail(0x41575ede, 'LLRBEmptyNode has no color.');
  }
  get left(): never {
    throw fail(0x741e2acf, 'LLRBEmptyNode has no left child.');
  }
  get right(): never {
    throw fail(0x901e9907, 'LLRBEmptyNode has no right child.');
  }
  size = 0;
