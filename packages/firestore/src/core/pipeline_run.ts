...
function evaluateSort(
  ...
  return input.sort((left, right): number => {
    ...
    // the documents
    for (const ordering of stage.orderings) {
...
function evaluateDocuments(
  ...
  if (stage.formattedPaths && stage.formattedPaths.length === 0) {
  ...
  const uniqueDocPaths = new Set(stage.formattedPaths);
  if (uniqueDocPaths.size !== stage.formattedPaths.length) {
    throw new FirestoreError(
      Code.INVALID_ARGUMENT,
  ...
  return input.filter(input => {
    return (
      input.isFoundDocument() &&
      stage.formattedPaths.includes(input.key.path.toStringWithLeadingSlash())
    );
  });
}
