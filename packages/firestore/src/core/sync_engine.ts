...
export class SyncEngine implements RemoteSyncer, SharedClientStateSyncer {
  ...
  async unlisten(query: Query): Promise<void> {
    ...
    const queryView = this.queryViewsByQuery.get(query)!;
    softAssert(!!queryView, 'Trying to unlisten on query not found:' + query);
    ...
    for (const targetId of targets) {
      ...
      if (queries && queries.length !== 0) {
        ...
        for (const query of queries) {
          const queryView = this.queryViewsByQuery.get(query);
          softAssert(!!queryView, `No query view found for ${query}`);
        ...
        softAssert(!!target, `Target for id ${targetId} not found`);
