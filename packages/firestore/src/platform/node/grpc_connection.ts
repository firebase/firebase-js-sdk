  ...
  openStream<Req, Resp>(
    ...
    const stream = new StreamBridge<Req, Resp>({
      sendFn: (msg: Req) => {
        if (!closed) {
          logDebug(
            LOG_TAG,
            `GRPC '${rpcName}' stream ${streamId} sending:`,
            msg
          );
          try {
      ...
      closeFn: () => {
        logDebug(
          LOG_TAG,
          `GRPC '${rpcName}' stream ${streamId} closed locally via close().`
        );
        close();
      }
    });
    ...
    grpcStream.on('data', (msg: Resp) => {
      if (!closed) {
        logDebug(
          LOG_TAG,
          `GRPC '${rpcName}' stream ${streamId} received:`,
          msg
        );
    ...
    grpcStream.on('end', () => {
      logDebug(LOG_TAG, `GRPC '${rpcName}' stream ${streamId} ended.`);
      close();
    });
    ...
    grpcStream.on('error', (grpcError: grpc.ServiceError) => {
      if (!closed) {
        logWarn(
          LOG_TAG,
          `GRPC '${rpcName}' stream ${streamId} error. Code:`,
          grpcError.code,
          'Message:',
          grpcError.message
        );
    ...
    logDebug(
      LOG_TAG,
      `Opening GRPC '${rpcName}' stream ${streamId} ` +
