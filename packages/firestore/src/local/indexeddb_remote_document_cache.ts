...
export class IndexedDbRemoteDocumentCache implements RemoteDocumentCache {
  ...
  applyChanges(
    ...
    changes.forEach((key, maybeDocument) => {
      if (maybeDocument) {
        ...
        promises.push(this.addEntry(transaction, key, doc));
      } else {
        promises.push(this.removeEntry(transaction, key));
      }
    });
    ...
    promises.push(this.updateMetadata(transaction, sizeDelta));
  ...
  newChangeBuffer(): RemoteDocumentChangeBuffer {
    return new RemoteDocumentChangeBuffer(this);
  }
