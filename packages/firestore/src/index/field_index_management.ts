...
export class FieldIndexManagementApiImpl implements FieldIndexManagementApi {
  ...
  private indexBackfillerScheduler?: IndexBackfillerScheduler;
  ...
  getIndexBackfillerScheduler(
    ...
    if (!this.indexBackfiller) {
      this.indexBackfiller = new IndexBackfiller(localStore, persistence, this);
    }
  ...
  createCacheIndexes(
    ...
    if (
      ...
      return this.createTargetIndexes(transaction, queryToTarget(query));
    }
  ...
  performQueryUsingIndex(
    ...
    return this.getIndexType(transaction, target).next(indexType => {
      ...
      return this.getDocumentsMatchingTarget(transaction, target).next(keys => {
        debugAssert(
          !!keys,
        ...
        return localDocumentsView
          ...
          .next(indexedDocuments => {
            return this.getMinOffset(transaction, target).next(offset => {
              const previousResults = applyQuery(query, indexedDocuments);

              if (
                needsRefill(query, previousResults, sortedKeys, offset.readTime)
              ) {
