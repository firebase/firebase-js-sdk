 ...
 *
 * Note: These refer strictly to the underlying evaluation type generated by the Firestore backend.
 ...
 * - Advanced configurations parsing backend types (such as `decimal128`, `max_key` or `min_key` from BSON)
 *   are also incorporated in this union string type. Note that `decimal128` is a backend-only
 *   numeric type that the JavaScript SDK cannot create natively, but can be evaluated in pipelines.
 */
export type Type =
  | 'null'
  | 'array'
...
export abstract class Expression implements ProtoValueSerializable, UserData {
   ...
   *
   * @remarks
   * Evaluated strictly on the backend as an AST. It will not execute via local conditional `if`
   * statements, but only inside pipeline evaluations (e.g., `where()`, `conditional()`). Null or
   * undefined fields evaluate to skip/error. Use `ifAbsent()` / `isAbsent()` to evaluate missing data.
   *
   * @example
   * ```typescript
