import firebase, { _getProvider, _removeServiceInstance, _registerComponent, registerVersion } from '@firebase/app';
    ...
    async runTransaction(mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$1, 'Transaction failed with error: %s. Retrying: %s.', error.message, retryable);
                this.close();
                if (!retryable) {
 ...
 * limitations under the License.
 */
const LOG_TAG$3 = 'SharedClientState';
/**
...
class MutationMetadata {
    ...
    static fromWebStorageEntry(user, batchId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$3, `Failed to parse mutation state for ID '${batchId}': ${value}`);
            return null;
        }
    }
    toWebStorageJSON() {
...
class QueryTargetMetadata {
    ...
    static fromWebStorageEntry(targetId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$3, `Failed to parse target state for ID '${targetId}': ${value}`);
            return null;
        }
    }
    toWebStorageJSON() {
...
class RemoteClientState {
    ...
    static fromWebStorageEntry(clientId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$3, `Failed to parse client data for instance '${clientId}': ${value}`);
            return null;
        }
    }
}
...
class SharedOnlineState {
    ...
    static fromWebStorageEntry(value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$3, `Failed to parse online state: ${value}`);
            return null;
        }
    }
}
...
class WebStorageSharedClientState {
    ...
    getItem(key) {
        ...
        logDebug(LOG_TAG$3, 'READ', key, value);
        return value;
    }
    setItem(key, value) {
        logDebug(LOG_TAG$3, 'SET', key, value);
        this.storage.setItem(key, value);
    }
    removeItem(key) {
        logDebug(LOG_TAG$3, 'REMOVE', key);
    ...
    handleWebStorageEvent(event) {
        ...
        if (storageEvent.storageArea === this.storage) {
            logDebug(LOG_TAG$3, 'EVENT', storageEvent.key, storageEvent.newValue);
    ...
    async handleMutationBatchEvent(mutationBatch) {
        if (mutationBatch.user.uid !== this.currentUser.uid) {
            logDebug(LOG_TAG$3, `Ignoring mutation for non-active user ${mutationBatch.user.uid}`);
            return;
        }
...
function fromWebStorageSequenceNumber(seqString) {
    ...
    if (seqString != null) {
        try {
            ...
            logError(LOG_TAG$3, 'Failed to read sequence number from WebStorage', e);
        }
    }
 ...
 * limitations under the License.
 */
const LOG_TAG$5 = 'LruGarbageCollector';
    ...
    scheduleGC(localStore) {
        ...
        this.gcTask = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection" /* LruGarbageCollection */, delay, async () => {
            ...
            try {
                ...
                if (isIndexedDbTransactionError(e)) {
                    logDebug(LOG_TAG$5, 'Ignoring IndexedDB error during garbage collection: ', e);
                }
                else {
 ...
 * limitations under the License.
 */
const LOG_TAG$6 = 'IndexedDbPersistence';
/**
...
class IndexedDbPersistence {
    ...
    forceOwningTab) {
        ...
        if (this.window && this.window.localStorage) {
            ...
            if (forceOwningTab === false) {
                logError(LOG_TAG$6, 'LocalStorage is unavailable. As a result, persistence may not work ' +
    ...
    updateClientMetadataAndTryBecomePrimary() {
        return this.runTransaction('updateClientMetadataAndTryBecomePrimary', 'readwrite', txn => {
            ...
            .catch(e => {
            if (isIndexedDbTransactionError(e)) {
                logDebug(LOG_TAG$6, 'Failed to extend owner lease: ', e);
            ...
            logDebug(LOG_TAG$6, 'Releasing owner lease after error during lease refresh', e);
    ...
    canActAsPrimary(txn) {
        ...
        return store
            ...
            .next(canActAsPrimary => {
            if (this.isPrimary !== canActAsPrimary) {
                logDebug(LOG_TAG$6, `Client ${canActAsPrimary ? 'is' : 'is not'} eligible for a primary lease.`);
            }
    ...
    releasePrimaryLeaseIfHeld(txn) {
        ...
        return store.get(DbPrimaryClient.key).next(primaryClient => {
            if (this.isLocalClient(primaryClient)) {
                logDebug(LOG_TAG$6, 'Releasing primary lease.');
    ...
    isClientZombied(clientId) {
        ...
        try {
            ...
            logDebug(LOG_TAG$6, `Client '${clientId}' ${isZombied ? 'is' : 'is not'} zombied in LocalStorage`);
            return isZombied;
        }
        catch (e) {
            // Gracefully handle if LocalStorage isn't working.
            logError(LOG_TAG$6, 'Failed to get zombied client id.', e);
            return false;
        }
    }
    /**
            ...
            promises.push(documentBuffer.getEntries(txn, updatedKeys).next(existingDocs => {
                remoteEvent.documentUpdates.forEach((key, doc) => {
                    ...
                    if (doc instanceof NoDocument &&
                        ...
                        logDebug(LOG_TAG$7, 'Ignoring outdated watch update for ', key, '. Current version:', existingDoc.version, ' Watch version:', doc.version);
                    }
...
async function ignoreIfPrimaryLeaseLoss(err) {
    if (err.code === Code.FAILED_PRECONDITION &&
        ...
        logDebug(LOG_TAG$7, 'Unexpectedly lost primary lease');
    }
    else {
        throw err;
    }
...
function enqueueWrite(asyncQueue, syncEngine, mutations) {
    const deferred = new Deferred();
    asyncQueue.enqueueAndForget(() => syncEngine.write(mutations, deferred));
    return deferred.promise;
}
function enqueueNetworkEnabled(asyncQueue, remoteStore, persistence, enabled) {
    return asyncQueue.enqueue(() => {
        persistence.setNetworkEnabled(enabled);
        return enabled ? remoteStore.enableNetwork() : remoteStore.disableNetwork();
    });
}
function enqueueWaitForPendingWrites(asyncQueue, syncEngine) {
    const deferred = new Deferred();
...
function enqueueSnapshotsInSyncListen(asyncQueue, eventManager, observer) {
    const wrappedObserver = new AsyncObserver(observer);
    asyncQueue.enqueueAndForget(async () => eventManager.addSnapshotsInSyncListener(wrappedObserver));
    return () => {
        wrappedObserver.mute();
        asyncQueue.enqueueAndForget(async () => eventManager.removeSnapshotsInSyncListener(wrappedObserver));
    };
}
 ...
 * limitations under the License.
 */
const LOG_TAG$e = 'ComponentProvider';
// settings() defaults:
...
function getDatastore(firestore) {
    ...
    if (!datastoreInstances.has(firestore)) {
        logDebug(LOG_TAG$e, 'Initializing Datastore');
...
function removeComponents(firestore) {
    ...
    if (datastore) {
        logDebug(LOG_TAG$e, 'Removing Datastore');
...
async function setOfflineComponentProvider(firestore, persistenceSettings, offlineComponentProvider) {
    ...
    offlineComponentProvider.persistence.setDatabaseDeletedListener(() => firestore.delete());
    offlineDeferred.resolve(offlineComponentProvider);
}
async function setOnlineComponentProvider(firestore, onlineComponentProvider) {
    ...
    firestore._setCredentialChangeListener(user => firestore._queue.enqueueAndForget(() => onlineComponentProvider.remoteStore.handleCredentialChange(user)));
    onlineDeferred.resolve(onlineComponentProvider);
}
function getOfflineComponentProvider(firestore) {
    ...
    if (!offlineComponentProviders.has(firestore)) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-floating-promises
        setOfflineComponentProvider(firestore, { durable: false }, new MemoryOfflineComponentProvider());
    }
    return offlineComponentProviders.get(firestore);
}
function getOnlineComponentProvider(firestore) {
    verifyNotTerminated(firestore);
...
function verifyNotTerminated(firestore) {
    if (firestore._terminated) {
        throw new FirestoreError(Code.FAILED_PRECONDITION, 'The client has already been terminated.');
    }
}
function getSyncEngine(firestore) {
    return getOnlineComponentProvider(firestore).then(components => components.syncEngine);
}
function getRemoteStore(firestore) {
    return getOnlineComponentProvider(firestore).then(components => components.remoteStore);
}
function getEventManager(firestore) {
    return getOnlineComponentProvider(firestore).then(components => components.eventManager);
}
function getPersistence(firestore) {
    return getOfflineComponentProvider(firestore).then(components => components.persistence);
}
function getLocalStore(firestore) {
    return getOfflineComponentProvider(firestore).then(provider => provider.localStore);
}
/**
 * Removes all components associated with the provided instance. Must be called
 * when the Firestore instance is terminated.
 */
async function removeComponents$1(firestore) {
    const onlineComponentProviderPromise = onlineComponentProviders.get(firestore);
    if (onlineComponentProviderPromise) {
        logDebug(LOG_TAG$f, 'Removing OnlineComponentProvider');
        onlineComponentProviders.delete(firestore);
        await (await onlineComponentProviderPromise).terminate();
    }
    const offlineComponentProviderPromise = offlineComponentProviders.get(firestore);
    if (offlineComponentProviderPromise) {
        logDebug(LOG_TAG$f, 'Removing OfflineComponentProvider');
 ...
 * limitations under the License.
 */
const LOG_TAG$g = 'Firestore';
/**
 * The root reference to the Firestore database and the entry point for the
 * tree-shakeable SDK.
 */
class Firestore$1 extends Firestore {
    constructor(app, authProvider) {
        super(app, authProvider);
        this._queue = new AsyncQueue();
        this._clientId = AutoId.newId();
        this._receivedInitialUser = new Deferred();
        this._user = User.UNAUTHENTICATED;
        this._credentialListener = () => { };
    ...
    _setCredentialChangeListener(credentialListener) {
        logDebug(LOG_TAG$g, 'Registering credential change listener');
        this._credentialListener = credentialListener;
        // eslint-disable-next-line @typescript-eslint/no-floating-promises
        this._receivedInitialUser.promise.then(() => this._credentialListener(this._user));
    }
    async _getConfiguration() {
        var _a, _b;
        const settings = this._getSettings();
        await this._receivedInitialUser.promise;
        const databaseInfo = new DatabaseInfo(this._databaseId, this._persistenceKey, (_a = settings.host) !== null && _a !== void 0 ? _a : DEFAULT_HOST, (_b = settings.ssl) !== null && _b !== void 0 ? _b : DEFAULT_SSL, 
        /* forceLongPolling= */ false);
        return {
...
function enableIndexedDbPersistence(firestore) {
    const firestoreImpl = cast(firestore, Firestore$1);
    ...
    return setOfflineComponentProvider(firestoreImpl, {
        durable: true,
        synchronizeTabs: false,
        cacheSizeBytes: settings.cacheSizeBytes || LruParams.DEFAULT_CACHE_SIZE_BYTES,
...
function enableMultiTabIndexedDbPersistence(firestore) {
    ...
    return setOfflineComponentProvider(firestoreImpl, {
        ...
        cacheSizeBytes: settings.cacheSizeBytes || LruParams.DEFAULT_CACHE_SIZE_BYTES,
        forceOwningTab: false
    }, offlineComponentProvider).then(() => setOnlineComponentProvider(firestoreImpl, onlineComponentProvider));
}
function clearIndexedDbPersistence(firestore) {
    ...
    firestoreImpl._queue.enqueueAndForgetEvenWhileRestricted(async () => {
        try {
            await indexedDbClearPersistence(indexedDbStoragePrefix(firestoreImpl._databaseId, firestoreImpl._persistenceKey));
            deferred.resolve();
        }
        catch (e) {
            deferred.reject(e);
...
function waitForPendingWrites(firestore) {
    const firestoreImpl = cast(firestore, Firestore$1);
    return getSyncEngine(firestoreImpl).then(syncEngine => enqueueWaitForPendingWrites(firestoreImpl._queue, syncEngine));
}
function enableNetwork(firestore) {
    ...
    ]).then(([remoteStore, persistence]) => enqueueNetworkEnabled(firestoreImpl._queue, remoteStore, persistence, 
    /* enabled= */ true));
}
...
function registerFirestore() {
    _registerComponent(new Component('firestore-exp', container => {
        ...
        return ((app, auth) => new Firestore$1(app, auth))(app, container.getProvider('auth-internal'));
    }, "PUBLIC" /* PUBLIC */));
    registerVersion('firestore-exp', version$1, 'node');
}
 ...
 * limitations under the License.
 */
const DEFAULT_SERVER_TIMESTAMP_BEHAVIOR = 'none';
class DocumentSnapshot$1 extends DocumentSnapshot {
    constructor(_firestore, key, document, metadata, converter) {
        super(_firestore, key, document, converter);
        this._firestore = _firestore;
        this.metadata = metadata;
        this._firestoreImpl = cast(_firestore, Firestore$1);
    }
    exists() {
        return super.exists();
    }
    data(options) {
        if (!this._document) {
            ...
            // We only want to use the converter and create a new DocumentSnapshot
            // if a converter has been provided.
            const snapshot = new QueryDocumentSnapshot$1(this._firestore, this._key, this._document, this.metadata, 
            /* converter= */ null);
            return this._converter.fromFirestore(snapshot, options);
        }
        else {
            const userDataWriter = new UserDataWriter(this._firestoreImpl._databaseId, 
            /* timestampsInSnapshots= */ true, (options === null || options === void 0 ? void 0 : options.serverTimestamps) || DEFAULT_SERVER_TIMESTAMP_BEHAVIOR, key => new DocumentReference(this._firestore, 
            /* converter= */ null, key.path));
            return userDataWriter.convertValue(this._document.toProto());
        }
    }
    get(fieldPath, options = {}) {
        if (this._document) {
            ...
            if (value !== null) {
                const userDataWriter = new UserDataWriter(this._firestoreImpl._databaseId, 
                /* timestampsInSnapshots= */ true, options.serverTimestamps || DEFAULT_SERVER_TIMESTAMP_BEHAVIOR, key => new DocumentReference(this._firestore, this._converter, key.path));
                return userDataWriter.convertValue(value);
            }
        }
        return undefined;
...
class QuerySnapshot {
    constructor(_firestore, query, _snapshot) {
        this._firestore = _firestore;
        this.query = query;
    ...
    forEach(callback, thisArg) {
        this._snapshot.docs.forEach(doc => {
            callback.call(thisArg, this._convertToDocumentSnapshot(doc, this._snapshot.fromCache, this._snapshot.mutatedKeys.has(doc.key)));
        });
    }
    docChanges(options = {}) {
        ...
        if (!this._cachedChanges ||
            ...
            this._cachedChanges = changesFromSnapshot(this._snapshot, includeMetadataChanges, this._convertToDocumentSnapshot.bind(this));
            this._cachedChangesIncludeMetadataChanges = includeMetadataChanges;
        }
        return this._cachedChanges;
    }
    _convertToDocumentSnapshot(doc, fromCache, hasPendingWrites) {
        return new QueryDocumentSnapshot$1(this._firestore, doc.key, doc, new SnapshotMetadata(hasPendingWrites, fromCache), this.query.converter);
    }
}
...
class Transaction$2 extends Transaction$1 {
    ...
    get(documentRef) {
        ...
        return super
            .get(documentRef)
            .then(liteDocumentSnapshot => new DocumentSnapshot$1(this._firestore, ref._key, liteDocumentSnapshot._document, new SnapshotMetadata(
        /* hasPendingWrites= */ false, 
        /* fromCache= */ false), ref._converter));
    }
}
function runTransaction(firestore, updateFunction) {
    const firestoreClient = cast(firestore, Firestore$1);
    const datastore = getDatastore(firestoreClient);
    const deferred = new Deferred();
    new TransactionRunner(new AsyncQueue(), datastore, internalTransaction => updateFunction(new Transaction$2(firestoreClient, internalTransaction)), deferred).run();
    return deferred.promise;
}
...
function getDoc(reference) {
    ...
    const firestore = cast(ref.firestore, Firestore$1);
    return getEventManager(firestore).then(eventManager => enqueueReadDocumentViaSnapshotListener(firestore._queue, eventManager, ref._key).then(doc => convertToDocSnapshot(firestore, ref, doc)));
}
function getDocFromCache(reference) {
    const ref = cast(reference, DocumentReference);
    const firestore = cast(ref.firestore, Firestore$1);
    return getLocalStore(firestore).then(localStore => enqueueReadDocumentFromCache(firestore._queue, localStore, ref._key).then(doc => new DocumentSnapshot$1(firestore, ref._key, doc, new SnapshotMetadata(doc instanceof Document ? doc.hasLocalMutations : false, 
    /* fromCache= */ true), ref._converter)));
}
function getDocFromServer(reference) {
    const ref = cast(reference, DocumentReference);
    const firestore = cast(ref.firestore, Firestore$1);
    return getEventManager(firestore).then(eventManager => enqueueReadDocumentViaSnapshotListener(firestore._queue, eventManager, ref._key, { source: 'server' }).then(viewSnapshot => convertToDocSnapshot(firestore, ref, viewSnapshot)));
}
function getDocs(query) {
    const internalQuery = cast(query, Query);
    const firestore = cast(query.firestore, Firestore$1);
    validateHasExplicitOrderByForLimitToLast(internalQuery._query);
    return getEventManager(firestore).then(eventManager => enqueueExecuteQueryViaSnapshotListener(firestore._queue, eventManager, internalQuery._query).then(snapshot => new QuerySnapshot(firestore, internalQuery, snapshot)));
}
function getDocsFromCache(query) {
    const internalQuery = cast(query, Query);
    const firestore = cast(query.firestore, Firestore$1);
    return getLocalStore(firestore).then(localStore => enqueueExecuteQueryFromCache(firestore._queue, localStore, internalQuery._query).then(snapshot => new QuerySnapshot(firestore, internalQuery, snapshot)));
}
function getDocsFromServer(query) {
    const internalQuery = cast(query, Query);
    const firestore = cast(query.firestore, Firestore$1);
    return getEventManager(firestore).then(eventManager => enqueueExecuteQueryViaSnapshotListener(firestore._queue, eventManager, internalQuery._query, { source: 'server' }).then(snapshot => new QuerySnapshot(firestore, internalQuery, snapshot)));
}
function setDoc(reference, data, options) {
    ...
    return getSyncEngine(firestore).then(syncEngine => enqueueWrite(firestore._queue, syncEngine, parsed.toMutations(ref._key, Precondition.none())));
}
function updateDoc(reference, fieldOrUpdateData, value, ...moreFieldsAndValues) {
    ...
    return getSyncEngine(firestore).then(syncEngine => enqueueWrite(firestore._queue, syncEngine, parsed.toMutations(ref._key, Precondition.exists(true))));
}
function deleteDoc(reference) {
    const ref = cast(reference, DocumentReference);
    const firestore = cast(ref.firestore, Firestore$1);
    return getSyncEngine(firestore).then(syncEngine => enqueueWrite(firestore._queue, syncEngine, [
        new DeleteMutation(ref._key, Precondition.none())
    ]));
}
function addDoc(reference, data) {
    const collRef = cast(reference, CollectionReference);
    const firestore = cast(collRef.firestore, Firestore$1);
    ...
    const parsed = parseSetData(dataReader, 'addDoc', docRef._key, convertedValue, collRef.converter !== null, {});
    return getSyncEngine(firestore)
        .then(syncEngine => enqueueWrite(firestore._queue, syncEngine, parsed.toMutations(docRef._key, Precondition.exists(false))))
        .then(() => docRef);
}
function onSnapshot(ref, ...args) {
    ...
    let asyncUnsubscribe;
    if (ref instanceof DocumentReference) {
        const firestore = cast(ref.firestore, Firestore$1);
        const observer = {
            next: snapshot => {
                if (args[currArg]) {
                    args[currArg](convertToDocSnapshot(firestore, ref, snapshot));
                }
            },
            error: args[currArg + 1],
            complete: args[currArg + 2]
        };
        asyncUnsubscribe = getEventManager(firestore).then(eventManager => enqueueListen(firestore._queue, eventManager, newQueryForPath(ref._key.path), internalOptions, observer));
    }
    else {
        const query = cast(ref, Query);
        const firestore = cast(query.firestore, Firestore$1);
        const observer = {
            next: snapshot => {
                if (args[currArg]) {
                    args[currArg](new QuerySnapshot(firestore, query, snapshot));
                }
            },
            error: args[currArg + 1],
            complete: args[currArg + 2]
        ...
        asyncUnsubscribe = getEventManager(firestore).then(eventManager => enqueueListen(firestore._queue, eventManager, query._query, internalOptions, observer));
    }
...
function onSnapshotsInSync(firestore, arg) {
    ...
    const asyncObserver = getEventManager(firestoreImpl).then(eventManager => enqueueSnapshotsInSyncListen(firestoreImpl._queue, eventManager, observer));
    // TODO(firestorexp): Add test that verifies that we don't raise a snapshot if
    // unsubscribe is called before `asyncObserver` resolves.
    return () => {
        // eslint-disable-next-line @typescript-eslint/no-floating-promises
...
function convertToDocSnapshot(firestore, ref, snapshot) {
    ...
    return new DocumentSnapshot$1(firestore, ref._key, doc, new SnapshotMetadata(snapshot.hasPendingWrites, snapshot.fromCache), ref._converter);
}
...
function writeBatch(firestore) {
    const firestoreImpl = cast(firestore, Firestore$1);
    return new WriteBatch(firestoreImpl, writes => getSyncEngine(firestoreImpl).then(syncEngine => enqueueWrite(firestoreImpl._queue, syncEngine, writes)));
}
