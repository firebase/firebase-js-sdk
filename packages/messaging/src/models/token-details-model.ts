...
export class TokenDetailsModel extends DbInterface {
  ...
  async getTokenDetailsFromToken(
    ...
    if (!fcmToken) {
      throw errorFactory.create(ErrorCodes.BAD_TOKEN);
    }
  ...
  async getTokenDetailsFromSWScope(
    ...
    if (!swScope) {
      throw errorFactory.create(ErrorCodes.BAD_SCOPE);
    }
  ...
  async saveTokenDetails(tokenDetails: TokenDetails): Promise<void> {
    ...
    if (!tokenDetails.vapidKey) {
      throw errorFactory.create(ErrorCodes.BAD_VAPID_KEY);
    }

    if (!tokenDetails.endpoint || !tokenDetails.auth || !tokenDetails.p256dh) {
      throw errorFactory.create(ErrorCodes.BAD_SUBSCRIPTION);
    }

    if (!tokenDetails.fcmSenderId) {
      throw errorFactory.create(ErrorCodes.BAD_SENDER_ID);
    }
    ...
    if (!tokenDetails.fcmPushSet) {
      throw errorFactory.create(ErrorCodes.BAD_PUSH_SET);
    }
  ...
  async deleteToken(token: string): Promise<TokenDetails> {
    if (typeof token !== 'string' || token.length === 0) {
      return Promise.reject(
        errorFactory.create(ErrorCodes.INVALID_DELETE_TOKEN)
      );
    }

    const details = await this.getTokenDetailsFromToken(token);
    if (!details) {
      throw errorFactory.create(ErrorCodes.DELETE_TOKEN_NOT_FOUND);
    }
