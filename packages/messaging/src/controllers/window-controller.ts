...
export class WindowController extends BaseController {
  ...
  async requestPermission(): Promise<void> {
    ...
    if (permissionResult === 'granted') {
      ...
      throw errorFactory.create(ErrorCodes.PERMISSION_BLOCKED);
    } else {
      throw errorFactory.create(ErrorCodes.PERMISSION_DEFAULT);
    }
  }
  ...
  useServiceWorker(registration: ServiceWorkerRegistration): void {
    ...
    if (this.registrationToUse != null) {
      throw errorFactory.create(ErrorCodes.USE_SW_BEFORE_GET_TOKEN);
    }
  ...
  usePublicVapidKey(publicKey: string): void {
    if (typeof publicKey !== 'string') {
      throw errorFactory.create(ErrorCodes.INVALID_PUBLIC_VAPID_KEY);
    }

    if (this.publicVapidKeyToUse != null) {
      throw errorFactory.create(ErrorCodes.USE_PUBLIC_KEY_BEFORE_GET_TOKEN);
    }

    const parsedKey = base64ToArrayBuffer(publicKey);

    if (parsedKey.length !== 65) {
      throw errorFactory.create(ErrorCodes.PUBLIC_KEY_DECRYPTION_FAILED);
    }
  ...
  waitForRegistrationToActivate_(
    ...
    return new Promise<ServiceWorkerRegistration>((resolve, reject) => {
      if (!serviceWorker) {
        ...
        reject(errorFactory.create(ErrorCodes.NO_SW_IN_REG));
        return;
      }
      ...
      if (serviceWorker.state === 'redundant') {
        reject(errorFactory.create(ErrorCodes.SW_REG_REDUNDANT));
        return;
      }
  ...
  getSWRegistration_(): Promise<ServiceWorkerRegistration> {
    ...
    return navigator.serviceWorker
      ...
      .catch((err: Error) => {
        throw errorFactory.create(ErrorCodes.FAILED_DEFAULT_REGISTRATION, {
...
export async function manifestCheck(): Promise<void> {
  ...
  if (manifestContent.gcm_sender_id !== '103953800507') {
    throw errorFactory.create(ErrorCodes.INCORRECT_GCM_SENDER_ID);
  }
}
