...
export interface Settings {
  ...
  /**
   * Enables the use of {@link Timestamp}s for timestamp fields in {@link
   * DocumentSnapshot}s.
   *
   ...
   * causes unexpected behavior when using a timestamp from a snapshot as a part
   * of a subsequent query.
   *
   * Setting {@link timestampsInSnapshotsEnabled} to true will cause Firestore
   * to return {@link Timestamp} values instead of {@link Date} avoiding this
   * kind of problem. To make this work you must also change any code that uses
   * {@link Date} to use {@link Timestamp} instead.
   *
   * NOTE: in the future `timestampsInSnapshotsEnabled = true` will become the
