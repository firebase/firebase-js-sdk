...
import { RemoteConfig as RemoteConfigImpl } from '../src/remote_config'; // Import RemoteConfigImpl for type casting
import { ConfigUpdateObserver } from '../src/public_types'; // Required for real-time observer and FetchResponse type
...
describe('Remote Config API', () => {
  ...
  describe('onConfigUpdate', () => {
    ...
    beforeEach(() => {
      ...
      addObserverStub = sinon.stub(rc._realtimeHandler, 'addObserver').resolves();
      removeObserverStub = sinon.stub(rc._realtimeHandler, 'removeObserver').resolves();
    ...
    it('should call addObserver on the internal realtimeHandler', async () => {
      const mockObserver = { next: sinon.stub(), error: sinon.stub(), complete: sinon.stub() };
    ...
    it('observer.next should be called when realtimeHandler propagates an update', async () => {
      ...
      expect(mockObserver.next.getCall(0).args[0].getUpdatedKeys()).to.deep.equal(new Set(['new_key']));
    });
     ...
     it('observer.error should be called when realtimeHandler propagates an error', async () => {
      ...
      if (capturedObserver && capturedObserver.error) {
        ...
        const mockError = ERROR_FACTORY.create(ErrorCode.CONFIG_UPDATE_STREAM_ERROR, {
      ...
      expect(receivedError.message).to.equal('Remote Config: The stream was not able to connect to the backend. (remoteconfig/stream-error).');
      expect(receivedError).to.have.nested.property('customData.originalErrorMessage', 'Realtime stream error');
