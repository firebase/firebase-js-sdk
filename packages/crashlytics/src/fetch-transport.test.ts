    ...
    it('returns success when request succeeds', done => {
      ...
      transport.send(testPayload, requestTimeout).then(response => {
        ...
        try {
          ...
          const sentHeaders = fetchStub.firstCall.args[1]?.headers as Headers;
          assert.strictEqual(sentHeaders?.get('foo'), 'foo-value');
          assert.strictEqual(sentHeaders?.get('bar'), 'bar-value');
          assert.strictEqual(
    ...
    it('attaches static and dynamic headers to the request', done => {
      ...
      transport.send(testPayload, requestTimeout).then(response => {
        ...
        try {
          ...
          assert.strictEqual(
            sentHeaders?.get('dynamic-header'),
            'dynamic-value'
          );
          done();
        } catch (e) {
