...
const allBuilds = [
  ...
  // which add build target reporting
  {
    ...
    output: {
      file: path.resolve('./react', pkg['main-esm']),
      format: 'es',
      sourcemap: true
    },
    plugins: [
      alias(util.generateAliasConfig('node_react')),
      ...nodePlugins(),
      replace({
  ...
  // Node CJS build
  {
    input: path.resolve('./react', pkg['main-esm']),
    output: {
      file: path.resolve('./react', pkg.main),
      format: 'cjs',
      sourcemap: true
    },
    plugins: [
  ...
  // which add build target reporting
  {
    input: './react/index.tsx',
    output: {
      file: path.resolve('./react', pkg.browser),
      format: 'es',
      sourcemap: true
    },
    plugins: [
      alias(util.generateAliasConfig('browser_react')),
      ...browserPlugins(),
  ...
  {
    input: path.resolve('./react', pkg.browser),
    output: [
      {
        file: path.resolve('./react', pkg.esm5),
        format: 'es',
        sourcemap: true
      }
    ],
  ...
  // RN build
  {
    ...
    output: {
      file: path.resolve('./react', pkg['react-native']),
      format: 'es',
      sourcemap: true
    },
    plugins: [
      alias(util.generateAliasConfig('rn_react')),
      ...browserPlugins(),
      replace({
