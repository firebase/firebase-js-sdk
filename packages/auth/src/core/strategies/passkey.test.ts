...
describe('passkey', async () => {
  ...
  it('should sign out if fallback passkey enrollment fails and deleting the anonymous user fails', async () => {
    if (
      typeof navigator === 'undefined' ||
      typeof navigator.credentials === 'undefined'
    ) {
    ...
    const deleteMock = mockEndpoint(
      Endpoint.DELETE_ACCOUNT,
      { error: { message: 'USER_NOT_FOUND' } },
      400
    );
    const signOutSpy = sinon.spy(auth, 'signOut');
