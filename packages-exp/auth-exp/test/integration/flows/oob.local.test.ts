...
describe.only('Integration test: oob codes', () => {
  ...
  async function code(toEmail: string): Promise<OobCodeSession> {
    return (await getOobCodes(auth))[toEmail];
  }
  ...
  context('flows beginning with sendSignInLinkToEmail', () => {
    ...
    it('allows user to sign in', async () => {
      const {user, operationType} = await signInWithEmailLink(auth, email, oobSession.oobLink);
    ...
    it('sign in works with an email credential', async () => {
      const cred = EmailAuthProvider.credentialWithLink(email, oobSession.oobLink);
    ...
    it('reauthenticate works with email credential', async () => {
      let cred = EmailAuthProvider.credentialWithLink(email, oobSession.oobLink);
      ...
      const {user: newUser, operationType} = await reauthenticateWithCredential(oldUser, cred);
    ...
    it('reauthenticate throws with different email', async () => {
      ...
      cred = EmailAuthProvider.credentialWithLink(newEmail, reauthSession.oobLink);
      await expect(reauthenticateWithCredential(oldUser, cred)).to.be.rejectedWith(FirebaseError, 'auth/user-mismatch');
    ...
    it('other accounts can be linked', async () => {
      ...
      const {user: linked, operationType} = await linkWithCredential(original, cred);
    ...
    it('can be linked to a custom token', async () => {
      const {user: original} = await signInWithCustomToken(auth, JSON.stringify({
        uid: 'custom-uid',
      }));
    ...
    it('cannot link if original account is deleted', async () => {
      ...
      await expect(linkWithCredential(user, cred)).to.be.rejectedWith(FirebaseError, 'auth/user-token-expired');
    });

    it('code can only be used once', async () => {
      const link = oobSession.oobLink;
      await signInWithEmailLink(auth, email, link);
      await expect(signInWithEmailLink(auth, email, link)).to.be.rejectedWith(FirebaseError, 'auth/invalid-action-code');
    });

    it('fetchSignInMethodsForEmail returns the correct values', async () => {
      const {user} = await signInWithEmailLink(auth, email, oobSession.oobLink);
      expect(await fetchSignInMethodsForEmail(auth, email)).to.eql([SignInMethod.EMAIL_LINK]);
    ...
    it('throws an error if the wrong code is provided', async () => {
      ...
      await expect(signInWithEmailLink(auth, email, otherSession.oobLink)).to.be.rejectedWith(FirebaseError, 'auth/invalid-email');
    });
  });
  ...
  it('can be used to verify email', async () => {
    ...
    expect(await fetchSignInMethodsForEmail(auth, email)).to.eql([SignInMethod.EMAIL_PASSWORD]);
  ...
  it('can be used to initiate password reset', async () => {
    const {user: original} = await createUserWithEmailAndPassword(auth, email, 'password');
    ...
    const {user} = await signInWithEmailAndPassword(auth, email, 'new-password');
  ...
  xit('verifyBeforeUpdateEmail waits until flow completes', async () => {
    ...
    const {user} = await signInWithEmailLink(auth, email, (await code(email)).oobLink);
    ...
    await expect(signInWithEmailAndPassword(auth, email, 'password')).to.be.rejectedWith(FirebaseError, 'auth/alskdjf');
    const {user: newSignIn} = await signInWithEmailAndPassword(auth, updatedEmail, 'password');
