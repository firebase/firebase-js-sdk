...
async function extractDependenciesAndSize(exportName, jsBundle, map) {
    const input = tmp_1.fileSync().name + '.js';
    const externalDepsResolvedOutput = tmp_1.fileSync().name + '.js';
    const externalDepsNotResolvedOutput = tmp_1.fileSync().name + '.js';
    const exportStatement = `export { ${exportName} } from '${path_1.resolve(jsBundle)}';`;
    fs_1.writeFileSync(input, exportStatement);
    // Run Rollup on the JavaScript above to produce a tree-shaken build
    const externalDepsResolvedBundle = await rollup_1.rollup({
        input,
        plugins: [
            rollup_plugin_node_resolve_1.resolve({
    ...
    const externalDepsNotResolvedBundle = await rollup_1.rollup({
        input
    });
    ...
    const externalDepsResolvedOutputContent = fs_1.readFileSync(externalDepsResolvedOutput, 'utf-8');
    // Extract size of minified build
    const externalDepsNotResolvedOutputContent = fs_1.readFileSync(externalDepsNotResolvedOutput, 'utf-8');
    const externalDepsResolvedOutputContentMinimized = terser_1.minify(externalDepsResolvedOutputContent, {
        output: {
            comments: false
        },
        mangle: true,
    ...
    const externalDepsNotResolvedOutputContentMinimized = terser_1.minify(externalDepsNotResolvedOutputContent, {
        output: {
            comments: false
        },
        mangle: true,
    ...
    fs_1.unlinkSync(input);
    fs_1.unlinkSync(externalDepsNotResolvedOutput);
    fs_1.unlinkSync(externalDepsResolvedOutput);
    return {
        dependencies,
        sizeInBytes: Buffer.byteLength(externalDepsNotResolvedOutputContentMinimized.code, 'utf-8'),
        sizeInBytesWithExternalDeps: Buffer.byteLength(externalDepsResolvedOutputContentMinimized.code, 'utf-8')
    };
}
exports.extractDependenciesAndSize = extractDependenciesAndSize;
/**
...
function extractDeclarations(filePath, map) {
    const program = typescript_1.createProgram([filePath], { allowJs: true });
    ...
    typescript_1.forEachChild(sourceFile, node => {
        if (typescript_1.isFunctionDeclaration(node)) {
            declarations.functions.push(node.name.text);
        }
        else if (typescript_1.isClassDeclaration(node)) {
            declarations.classes.push(node.name.text);
        }
        else if (typescript_1.isVariableDeclaration(node)) {
            declarations.variables.push(node.name.getText());
        }
        else if (typescript_1.isEnumDeclaration(node)) {
            ...
            variableDeclarations.forEach(variableDeclaration => {
                ...
                if (typescript_1.isIdentifier(variableDeclaration.name)) {
        ...
        else if (typescript_1.isImportDeclaration(node) && node.importClause) {
            ...
            if (symbol && symbol.valueDeclaration) {
                ...
                if (node.importClause.namedBindings &&
                    typescript_1.isNamedImports(node.importClause.namedBindings)) {
                    ...
                    typescript_1.isNamespaceImport(node.importClause.namedBindings)) {
                    ...
                    typescript_1.isIdentifier(node.importClause.name)) {
        ...
        // export {foo as foo1, bar} from '...';
            ...
            if (node.moduleSpecifier) {
                if (typescript_1.isStringLiteral(node.moduleSpecifier)) {
                ...
                const reExportsWithoutFromClause = handleExportStatementsWithoutFromClause(node, importSymbolCurrentNameToModuleLocation, importSymbolCurrentNameToOriginalName, importModuleLocationToExportedSymbolsList, namespaceImportSet);
                // concatenate re-exported MemberList with MemberList of the dts file
                for (const key of Object.keys(declarations)) {
...
function handleExportStatementsWithFromClause(checker, node, moduleName) {
    ...
    else if (node.exportClause && typescript_1.isNamedExports(node.exportClause)) {
...
function extractSymbolsFromNamedExportStatement(node, exportsFullList) {
    if (node.exportClause && typescript_1.isNamedExports(node.exportClause)) {
        const actualExports = [];
        node.exportClause.elements.forEach(exportSpecifier => {
 ...
 */
function handleExportStatementsWithoutFromClause(node, importSymbolCurrentNameToModuleLocation, importSymbolCurrentNameToOriginalName, importModuleLocationToExportedSymbolsList, namespaceImportSymbolSet) {
    const declarations = {
        functions: [],
        classes: [],
        variables: [],
    ...
    if (node.exportClause && typescript_1.isNamedExports(node.exportClause)) {
        node.exportClause.elements.forEach(exportSpecifier => {
            ...
            if (namespaceImportSymbolSet.has(exportSymbolOriginalName)) {
                ...
                replaceAll(declarations, exportSymbolOriginalName, exportSymbolCurrentName);
            }
            ...
            // export {A as AA , B as BB };
                ...
                if (importModuleLocationToExportedSymbolsList.has(moduleLocation)) {
                    reExportedSymbols = util_1.deepCopy(importModuleLocationToExportedSymbolsList.get(moduleLocation));
                }
                else {
                    reExportedSymbols = extractDeclarations(importSymbolCurrentNameToModuleLocation.get(exportSymbolOriginalName));
                    importModuleLocationToExportedSymbolsList.set(moduleLocation, util_1.deepCopy(reExportedSymbols));
                }
...
exports.mapSymbolToType = mapSymbolToType;
...
function writeReportToFile(report, outputFile) {
    if (fs_1.existsSync(outputFile) && !fs_1.lstatSync(outputFile).isFile()) {
        throw new Error('An output file is required but a directory given!' /* OUTPUT_FILE_REQUIRED */);
    }
    const directoryPath = path_1.dirname(outputFile);
    ...
    fs_1.writeFileSync(outputFile, report);
}
...
function writeReportToDirectory(report, fileName, directoryPath) {
    if (fs_1.existsSync(directoryPath) && !fs_1.lstatSync(directoryPath).isDirectory()) {
...
exports.writeReportToDirectory = writeReportToDirectory;
/**
...
function extractExternalDependencies(minimizedBundleFile) {
    const program = typescript_1.createProgram([minimizedBundleFile], { allowJs: true });
 ...
 */
async function generateReportForModule(moduleLocation, outputDirectory, writeFiles) {
    ...
    if (packageJson[TYPINGS]) {
        ...
        const fileName = `${path_1.basename(packageJson.name)}-dependency.json`;
        if (writeFiles) {
            writeReportToDirectory(json, fileName, path_1.resolve(outputDirectory));
        }
        return json;
    }
}
exports.generateReportForModule = generateReportForModule;
/**
 *
 ...
 */
function traverseDirs(moduleLocation, outputDirectory, writeFiles, executor, level, levelLimit) {
    if (level > levelLimit) {
        return;
    }
    executor(moduleLocation, outputDirectory, writeFiles);
    for (const name of fs_1.readdirSync(moduleLocation)) {
        const p = `${moduleLocation}/${name}`;
        if (fs_1.lstatSync(p).isDirectory()) {
            traverseDirs(p, outputDirectory, writeFiles, executor, level + 1, levelLimit);
        }
    }
}
/**
...
async function generateReport(dtsFile, bundleFile) {
    const resolvedDtsFile = path_1.resolve(dtsFile);
    const resolvedBundleFile = path_1.resolve(bundleFile);
    if (!fs_1.existsSync(resolvedDtsFile)) {
        throw new Error('Input dts file does not exist!' /* INPUT_DTS_FILE_DOES_NOT_EXIST */);
    }
    if (!fs_1.existsSync(resolvedBundleFile)) {
 ...
 *
 */
function generateReportForModules(moduleLocations, outputDirectory, writeFiles) {
    for (const moduleLocation of moduleLocations) {
        ...
        traverseDirs(moduleLocation, outputDirectory, writeFiles, generateReportForModule, 0, 1);
    }
}
exports.generateReportForModules = generateReportForModules;
