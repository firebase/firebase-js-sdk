...
async function generateDocs(
  ...
  // After all primary .api.json files have been generated by api-report and copied to tmpDir,
  // we merge secondary entry points (lite, pipelines, lite/pipelines, sw) into firestore.api.json
  // and messaging.api.json. This restores multi-entry-point devsite documentation without requiring
  // custom API Extractor schema extensions like the legacy "additionalEntryPoints" property.
  console.log(
...
/**
 * Merges secondary entry points (e.g., lite, pipelines, lite/pipelines, sw) into the primary
 * package `.api.json` models before documentation generation.
 *
 * Why this is needed:
 * Official @microsoft/api-extractor analyzes only one entry point (mainEntryPointFilePath) per config.
 * To document secondary entry points without duplicating shared SDK classes across pages:
 * 1. We run API Extractor against each secondary `.d.ts` file using a temporary config.
 * 2. We extract the generated ApiEntryPoint object and append it to `mainJson.members` (in API Extractor's
 *    schema, an ApiPackage's `members` array holds its ApiEntryPoint objects).
 * 3. When `api-documenter-devsite` processes a package with multiple ApiEntryPoint members, it generates
 *    grouped pages (`firestore_.*.md`, `firestore_lite.*.md`, etc.) and documents shared core classes
 *    (like CollectionReference) only under their defining entry point.
 */
