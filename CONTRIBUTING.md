...
Reference docs for the Firebase [JS SDK](https://firebase.google.com/docs/reference/js/) are
generated from [TSDoc](https://tsdoc.org/) comments on the public API in each package's source
code (for example, the docs for the `AIModel` class in `docs-devsite/ai.aimodel.md` come from
the comments on that class in `packages/ai/src/models/ai-model.ts`).
...
and should never be edited directly - as noted in the header of each generated file, any manual
change will be overwritten by the next generation pass.
...
2. Run `yarn docgen:all` in the root directory to regenerate the files in `docs-devsite/` (see
   [Generating Documentation HTML Files](#generating-documentation-html-files) below).
3. Commit both the source comment change and the regenerated `docs-devsite/` files in your PR.

The table of contents (`docs-devsite/_toc.yaml`) is also generated by `yarn docgen:all`, so
added or removed classes and interfaces are reflected automatically.

Docs for the legacy [compat API](https://firebase.google.com/docs/reference/js/v8/) are
generated separately by [Typedoc](https://typedoc.org/) from the
[firebase compat index.d.ts type definition file](packages/firebase/compat/index.d.ts) via
`yarn docgen:compat`, with its table of contents maintained in
[scripts/docgen-compat/content-sources/js/toc.yaml](scripts/docgen-compat/content-sources/js/toc.yaml).
The compat API is no longer under active development and documentation updates should generally
target the current modular API instead.
