!(function (e, t) {
    ...
    constructor(t, n, r) {
      (this.firebase_ = r),
        ...
        (this.automaticDataCollectionEnabled_ =
          n.automaticDataCollectionEnabled || !1),
        (this.options_ = e(void 0, t)),
        (this.container = new u(n.name)),
        this.container.addComponent(new s('app', () => this, 'PUBLIC'));
      for (const e of this.firebase_.INTERNAL.components.values())
      ...
      return new Promise(e => {
        ...
        .then(
          () => (
            ...
            Promise.all(this.container.getProviders().map(e => e.delete()))
          )
        )
        .then(() => {
          this.isDeleted_ = !0;
  ...
  function S(t) {
    const n = {},
      ...
      i = {
        __esModule: !0,
        initializeApp: function (e, r = {}) {
          if ('object' != typeof r || null === r) {
            r = { name: r };
          }
          const s = r;
          void 0 === s.name && (s.name = y);
          const { name: a } = s;
          if ('string' != typeof a || !a)
            throw b.create('bad-app-name', { appName: String(a) });
          if (o(n, a)) throw b.create('duplicate-app', { appName: a });
          const c = new t(e, s, i);
          return (n[a] = c), c;
        },
        app: a,
        registerVersion: function (e, t, n) {
          ...
          c(
            new s(i + '-version', () => ({ library: i, version: t }), 'VERSION')
          );
        },
        setLogLevel: v,
        onLog: function (e, t) {
          if (null !== e && 'function' != typeof e)
            throw b.create('invalid-log-argument', { appName: name });
          !(function (e, t) {
            for (const n of l) {
              let r = null;
    ...
    function a(e) {
      if (!o(n, (e = e || y))) throw b.create('no-app', { appName: e });
      return n[e];
    }
    function c(o) {
      const s = o.name;
      if (r.has(s))
        return (
          E.debug(`There were multiple attempts to register component ${s}.`),
          'PUBLIC' === o.type ? i[s] : null
        );
      if ((r.set(s, o), 'PUBLIC' === o.type)) {
        const n = (e = a()) => {
          if ('function' != typeof e[s])
            throw b.create('invalid-app-argument', { appName: s });
          return e[s]();
        };
        void 0 !== o.serviceProps && e(n, o.serviceProps),
          (i[s] = n),
          (t.prototype[s] = function (...e) {
            return this._getService
              .bind(this, s)
              .apply(this, o.multipleInstances ? e : []);
          });
      }
      for (const e of Object.keys(n)) n[e]._addComponent(o);
      return 'PUBLIC' === o.type ? i[s] : null;
    }
    return (
      (i.default = i),
  ...
  function U(e, t, n) {
    (this._db = e), (this.oldVersion = t), (this.transaction = new $(n));
  }
  function F(e) {
    this._db = e;
  }
  !(function (e, t) {
    e.INTERNAL.registerComponent(
      new s('platform-logger', e => new T(e), 'PRIVATE')
    ),
      e.registerVersion(w, '0.6.13', t),
      e.registerVersion('fire-js', '');
  })(A, 'lite'),
    D(j, '_index', ['name', 'keyPath', 'multiEntry', 'unique']),
    R(j, '_index', IDBIndex, [
      'get',
      'getKey',
      'getAll',
    ...
    P(j, '_index', IDBIndex, ['openCursor', 'openKeyCursor']),
    D(M, '_cursor', ['direction', 'key', 'primaryKey', 'value']),
    R(M, '_cursor', IDBCursor, ['update', 'delete']),
    ...
    (B.prototype.createIndex = function () {
    ...
    D(B, '_store', ['name', 'keyPath', 'indexNames', 'autoIncrement']),
    R(B, '_store', IDBObjectStore, [
      'put',
      'add',
      'delete',
      'clear',
    ...
    P(B, '_store', IDBObjectStore, ['openCursor', 'openKeyCursor']),
    L(B, '_store', IDBObjectStore, ['deleteIndex']),
    ($.prototype.objectStore = function () {
      return new B(this._tx.objectStore.apply(this._tx, arguments));
    }),
    D($, '_tx', ['objectStoreNames', 'mode']),
    L($, '_tx', IDBTransaction, ['abort']),
    (U.prototype.createObjectStore = function () {
      return new B(this._db.createObjectStore.apply(this._db, arguments));
    }),
    D(U, '_db', ['name', 'version', 'objectStoreNames']),
    L(U, '_db', IDBDatabase, ['deleteObjectStore', 'close']),
    (F.prototype.transaction = function () {
      return new $(this._db.transaction.apply(this._db, arguments));
    }),
    D(F, '_db', ['name', 'version', 'objectStoreNames']),
    L(F, '_db', IDBDatabase, ['close']),
    ['openCursor', 'openKeyCursor'].forEach(function (e) {
      [B, j].forEach(function (t) {
        e in t.prototype &&
  ...
  const x = '0.4.19',
    ...
    H = 'FIS_v2',
    K = 36e5,
    z = new r('installations', 'Installations', {
  ...
  function W(e) {
    return e instanceof n && e.code.includes('request-failed');
  }
  /**
   * @license
   * Copyright 2019 Google LLC
  ...
  async function Y(e, t) {
    ...
    return z.create('request-failed', {
      requestName: e,
      serverCode: n.code,
  ...
  function ce() {
    return (
      !ae &&
        ...
        ((ae = new BroadcastChannel('[Firebase] FID Change')),
        (ae.onmessage = e => {
  ...
  function fe() {
    return (
      pe ||
        (pe = (function (e, t, n) {
          var r = C(indexedDB, 'open', [e, t]),
            i = r.request;
          return (
            i &&
              (i.onupgradeneeded = function (e) {
                n && n(new U(i.result, e.oldVersion, i.transaction));
              }),
            r.then(function (e) {
              return new F(e);
            })
          );
        })('firebase-installations-database', 1, e => {
          switch (e.oldVersion) {
            case 0:
  ...
  async function he(e, t) {
    const n = re(e),
      r = (await fe()).transaction(le, 'readwrite'),
      i = r.objectStore(le),
      o = await i.get(n);
    return (
      await i.put(t, n),
  ...
  async function de(e) {
    const t = re(e),
      n = (await fe()).transaction(le, 'readwrite');
    await n.objectStore(le).delete(t), await n.complete;
  }
   ...
   */ async function me(e) {
    let t;
    const n = await ge(e, n => {
      const r = (function (e) {
          return we(e || { fid: ne(), registrationStatus: 0 });
        })(n),
        i = (function (e, t) {
          if (0 === t.registrationStatus) {
            if (!navigator.onLine) {
              return {
                installationEntry: t,
                registrationPromise: Promise.reject(z.create('app-offline'))
              };
            }
            const n = {
              fid: t.fid,
            ...
            return {
              ...
              registrationPromise: (async function (e, t) {
                try {
                  ...
                  throw (
                    (W(n) && 409 === n.customData.serverCode
                      ? await de(e)
                      : await he(e, { fid: t.fid, registrationStatus: 0 }),
                    n)
                  );
                }
              })(e, n)
  ...
  async function ve(e) {
    let t = await be(e);
    for (; 1 === t.registrationStatus; ) await ee(100), (t = await be(e));
    if (0 === t.registrationStatus) {
      const { installationEntry: t, registrationPromise: n } = await me(e);
      return n || t;
    }
    return t;
  }
  ...
  async function ye({ appConfig: e, platformLoggerProvider: t }, n) {
    ...
    const s = { installation: { sdkVersion: V } },
      a = { method: 'POST', headers: i, body: JSON.stringify(s) },
      c = await X(() => fetch(r, a));
    if (c.ok) {
      return J(await c.json());
    }
    throw await Y('Generate Auth Token', c);
  }
  async function _e(e, t = !1) {
    let n;
    const r = await ge(e.appConfig, r => {
      if (!Ee(r)) throw z.create('not-registered');
      const i = r.authToken;
      if (
        !t &&
        (function (e) {
      ...
      if (1 === i.requestStatus)
        return (
          (n = (async function (e, t) {
            ...
            const r = n.authToken;
            return 0 === r.requestStatus ? _e(e, t) : r;
          })(e, t)),
          r
        );
      {
        ...
        return (
          (n = (async function (e, t) {
            try {
              ...
              return await he(e.appConfig, r), n;
            } catch (n) {
              if (
                !W(n) ||
  ...
  function Ie(e) {
    return ge(e, e => {
      if (!Ee(e)) throw z.create('not-registered');
      const t = e.authToken;
      return 1 === (n = t).requestStatus && n.requestTime + q < Date.now()
  ...
  async function Se(e, t = !1) {
    await (async function (e) {
      const { registrationPromise: t } = await me(e);
      t && (await t);
    })(
      /**
       * @license
  ...
  async function Te(e, t) {
    const n = (function (e, { fid: t }) {
      ...
      r = { method: 'DELETE', headers: Q(e, t) },
      i = await X(() => fetch(n, r));
    if (!i.ok) throw await Y('Delete Installation', i);
  }
  /**
   * @license
   * Copyright 2019 Google LLC
   ...
   * limitations under the License.
   */
  function Ae({ appConfig: e }, t) {
    return (
      (function (e, t) {
        ce();
        const n = re(e);
        let r = ie.get(n);
        r || ((r = new Set()), ie.set(n, r)), r.add(t);
      })(e, t),
      () => {
        !(function (e, t) {
          const n = re(e),
            r = ie.get(n);
          r && (r.delete(t), 0 === r.size && ie.delete(n), ue());
        })(e, t);
      }
    );
  }
   ...
   * limitations under the License.
   */ var Oe;
  (Oe = A).INTERNAL.registerComponent(
    new s(
      'installations',
      e => {
        const t = e.getProvider('app').getImmediate(),
          n = {
            appConfig: (function (e) {
              ...
              for (const n of t) if (!e.options[n]) throw Ne(n);
              return {
                appName: e.name,
        ...
        return {
          ...
          getId: () =>
            (async function (e) {
              const { installationEntry: t, registrationPromise: n } = await me(
                e.appConfig
              );
              return (
                n ? n.catch(console.error) : _e(e).catch(console.error), t.fid
              );
            })(n),
          getToken: e => Se(n, e),
          delete: () =>
    ...
    Oe.registerVersion('@firebase/installations', x);
  const Ce = '0.4.4',
    ke = Ce,
    De = 'FB-PERF-TRACE-MEASURE',
    Re = '_wt_',
    Le = '_fcp',
    Pe = '_fid',
    je = '@firebase/performance/config',
    Me = '@firebase/performance/configexpire',
    Be = 'Performance',
    $e = new r('performance', Be, {
  ...
  class He {
    constructor(e) {
      if (((this.window = e), !e)) throw $e.create('no window');
    ...
    requiredApisAvailable() {
      return fetch && Promise && this.navigator && this.navigator.cookieEnabled
        ? ('indexedDB' in self && null != indexedDB) ||
            (Ue.info('IndexedDB is not supported by current browswer'), !1)
        : (Ue.info(
    ...
    setupObserver(e, t) {
      ...
      new this.PerformanceObserver(e => {
        for (const n of e.getEntries()) t(n);
      }).observe({ entryTypes: [e] });
    }
    static getInstance() {
      return void 0 === Fe && (Fe = new He(xe)), Fe;
    }
  }
  /**
   * @license
  ...
  function Ke(e, t) {
    ...
    if (n < 0 || n > 1) throw $e.create('invalid String merger input');
    const r = [];
    for (let n = 0; n < e.length; n++)
      r.push(e.charAt(n)), t.length > n && r.push(t.charAt(n));
    return r.join('');
  }
  /**
   * @license
   ...
   */ class ze {
    constructor() {
      (this.instrumentationEnabled = !0),
        ...
        (this.flTransportEndpointUrl = Ke(
    ...
    getProjectId() {
      ...
      if (!e) throw $e.create('no project id');
      return e;
    }
    getApiKey() {
      const e =
      ...
      if (!e) throw $e.create('no api key');
      return e;
    }
  ...
  const Je = ['firebase_', 'google_', 'ga_'],
    Ye = new RegExp('^[a-zA-Z]\\w*$');
  function Ze() {
    const e = He.getInstance().navigator;
    return 'serviceWorker' in e ? (e.serviceWorker.controller ? 2 : 3) : 1;
  }
  function Qe() {
    switch (He.getInstance().document.visibilityState) {
      case 'visible':
        return Ge.VISIBLE;
      case 'hidden':
        return Ge.HIDDEN;
  ...
  function Xe() {
    const e = He.getInstance().navigator.connection;
  ...
  const et = '0.0.1',
    tt = !0,
    nt = 'FIREBASE_INSTALLATIONS_AUTH';
  function rt(e) {
    const t = (function () {
      const e = He.getInstance().localStorage;
      if (!e) return;
      const t = e.getItem(Me);
      if (!(t && ((n = t), Number(n) > Date.now()))) return;
      var n;
      const r = e.getItem(je);
      if (!r) return;
      try {
        return JSON.parse(r);
      } catch (e) {
    ...
    return t
      ? (ot(t), Promise.resolve())
      : (function (e) {
          return (function () {
            const e = ze.getInstance().installationsService.getToken();
            return e.then(e => {}), e;
          })()
            .then(t => {
              const n = `https://firebaseremoteconfig.googleapis.com/v1/projects/${ze
                  .getInstance()
                  .getProjectId()}/namespaces/fireperf:fetch?key=${ze
                  .getInstance()
                  .getApiKey()}`,
                r = new Request(n, {
                  method: 'POST',
                  headers: { Authorization: `${nt} ${t}` },
                  body: JSON.stringify({
                    app_instance_id: e,
                    app_instance_id_token: t,
                    app_id: ze.getInstance().getAppId(),
                    app_version: ke,
                    sdk_version: et
                  })
                });
              return fetch(r).then(e => {
                ...
                throw $e.create('RC response not ok');
              });
            })
            .catch(() => {
              Ue.info(it);
          ...
          .then(
            e =>
              (function (e) {
                const t = He.getInstance().localStorage;
                if (!e || !t) return;
                t.setItem(je, JSON.stringify(e)),
                  t.setItem(
                    Me,
                    String(
                      Date.now() +
                        60 * ze.getInstance().configTimeToLive * 60 * 1e3
                    )
                  );
              })(e),
            () => {}
  ...
  const it = 'Could not fetch config, will use default configs';
  function ot(e) {
    if (!e) return e;
    ...
    return (
      void 0 !== n.fpr_enabled
        ? (t.loggingEnabled = 'true' === String(n.fpr_enabled))
        : (t.loggingEnabled = tt),
      ...
      (t.logTraceAfterSampling = st(t.tracesSamplingRate)),
      (t.logNetworkAfterSampling = st(t.networkRequestsSamplingRate)),
      e
    );
  }
  function st(e) {
  ...
  function ut() {
    return (
      ...
      (at =
        at ||
        (function () {
          const e = He.getInstance().document;
          return new Promise(t => {
            if (e && 'complete' !== e.readyState) {
              const n = () => {
                'complete' === e.readyState &&
                  (e.removeEventListener('readystatechange', n), t());
              };
          ...
          .then(() =>
            (function () {
              const e = ze.getInstance().installationsService.getId();
              return (
                e.then(e => {
                  Ve = e;
                }),
      ...
      if (0 !== ht)
        return dt.length
          ? void (function () {
              ...
              (function (e, t) {
                return (function (e) {
                  const t = ze.getInstance().getFlTransportFullUrl();
                  return fetch(t, { method: 'POST', body: JSON.stringify(e) });
                })(e)
                  .then(
                    e => (
                      e.ok || Ue.info('Call to Firebase backend failed.'),
                      e.json()
                    )
                  )
                  .then(e => {
                    ...
                    Array.isArray(i) &&
                      ...
                      ((dt = [...t, ...dt]),
                      Ue.info('Retry transport request later.')),
                      (ht = 3),
                      mt(r);
                  });
              })(
                {
                  ...
                  log_source: ze.getInstance().logSource,
                  log_event: t
                },
                e
              ).catch(() => {
  ...
  function vt(e) {
    return (...t) => {
      !(function (e) {
        if (!e.eventTime || !e.message) throw $e.create('invalid cc log');
        dt = [...dt, e];
  ...
  function wt(e) {
    ...
    (!t.instrumentationEnabled && e.isAuto) ||
      ((t.dataCollectionEnabled || e.isAuto) &&
        He.getInstance().requiredApisAvailable() &&
        ((e.isAuto && Qe() !== Ge.VISIBLE) ||
          (3 === ct
            ? yt(e)
            : ut().then(
                () => yt(e),
    ...
    return 0 === t
      ? (function (e) {
          const t = {
            ...
            n = { application_info: It(), network_request_metric: t };
      ...
      : (function (e) {
          ...
          const r = { application_info: It(), trace_metric: t };
          return JSON.stringify(r);
        })(e);
  }
  function It() {
    return {
      google_app_id: ze.getInstance().getAppId(),
      app_instance_id: We(),
      web_app_info: {
        sdk_version: ke,
        page_url: He.getInstance().getUrl(),
        service_worker_status: Ze(),
        visibility_state: Qe(),
        effective_connection_type: Xe()
      },
  ...
  class St {
    constructor(e, t = !1, n) {
      (this.name = e),
        ...
        this.isAuto ||
          ...
          (this.traceMeasure =
            n || `FB-PERF-TRACE-MEASURE-${this.randomId}-${this.name}`),
          n && this.calculateTraceMetrics());
    }
    start() {
      if (1 !== this.state)
        throw $e.create('trace started', { traceName: this.name });
      this.api.mark(this.traceStartMark), (this.state = 2);
    }
    stop() {
      if (2 !== this.state)
        throw $e.create('trace stopped', { traceName: this.name });
      (this.state = 3),
    ...
    record(e, t, n) {
      if (e <= 0)
        throw $e.create('nonpositive trace startTime', {
          traceName: this.name
        });
      if (t <= 0)
        throw $e.create('nonpositive trace duration', { traceName: this.name });
      if (
        ((this.durationUs = Math.floor(1e3 * t)),
        (this.startTimeUs = Math.floor(1e3 * e)),
        n &&
          n.attributes &&
          (this.customAttributes = Object.assign({}, n.attributes)),
        n && n.metrics)
      )
        for (const e of Object.keys(n.metrics))
          isNaN(Number(n.metrics[e])) ||
            (this.counters[e] = Number(Math.floor(n.metrics[e])));
      wt(this);
    }
      ...
      if (
        !(function (e, t) {
          return (
            ...
            ((t && t.startsWith(Re) && Et.indexOf(e) > -1) ||
              !e.startsWith('_'))
          );
        })(e, this.name)
      )
        throw $e.create('invalid custom metric name', { customMetricName: e });
      this.counters[e] = (function (e) {
        const t = Math.floor(e);
        return (
          t < e &&
            Ue.info(
              `Metric value should be an Integer, setting the value as : ${t}.`
            ),
          t
        );
      })(t);
    ...
    putAttribute(e, t) {
      const n = (function (e) {
          return (
            !(0 === e.length || e.length > 40) &&
            !Je.some(t => e.startsWith(t)) &&
            !!e.match(Ye)
          );
        })(e),
        r = (function (e) {
      ...
      if (n && r) this.customAttributes[e] = t;
      else {
        if (!n) throw $e.create('invalid attribute name', { attributeName: e });
        if (!r)
          throw $e.create('invalid attribute value', { attributeValue: t });
      }
    }
    getAttribute(e) {
    ...
    static createOobTrace(e, t, n) {
      const r = He.getInstance().getUrl();
      if (!r) return;
      const i = new St(Re + r, !0),
        o = Math.floor(1e3 * He.getInstance().getTimeOrigin());
      i.setStartTime(o),
        e &&
          e[0] &&
          (i.setDuration(Math.floor(1e3 * e[0].duration)),
          i.putMetric('domInteractive', Math.floor(1e3 * e[0].domInteractive)),
          i.putMetric(
            'domContentLoadedEventEnd',
            Math.floor(1e3 * e[0].domContentLoadedEventEnd)
          ),
          i.putMetric('loadEventEnd', Math.floor(1e3 * e[0].loadEventEnd)));
      if (t) {
        const e = t.find(e => 'first-paint' === e.name);
        e && e.startTime && i.putMetric('_fp', Math.floor(1e3 * e.startTime));
        const r = t.find(e => 'first-contentful-paint' === e.name);
        r && r.startTime && i.putMetric(Le, Math.floor(1e3 * r.startTime)),
          n && i.putMetric(Pe, Math.floor(1e3 * n));
      }
      wt(i);
    }
    static createUserTimingTrace(e) {
      wt(new St(e, !1, e));
    }
  }
  /**
   ...
   */ function Tt(e) {
    ...
    const n = He.getInstance().getTimeOrigin(),
      r = Math.floor(1e3 * (t.startTime + n)),
      i = t.responseStart
        ? Math.floor(1e3 * (t.responseStart - t.startTime))
        : void 0,
      o = Math.floor(1e3 * (t.responseEnd - t.startTime));
    !(function (e) {
      const t = ze.getInstance();
        ...
        () =>
          (function () {
            ...
            if (e.onFirstInputDelay) {
              let r = setTimeout(() => {
                St.createOobTrace(t, n), (r = void 0);
              }, 5e3);
              e.onFirstInputDelay(e => {
                r && (clearTimeout(r), St.createOobTrace(t, n, e));
              });
            } else St.createOobTrace(t, n);
          })(),
        0
      ),
      setTimeout(
        () =>
          (function () {
            ...
            for (const e of t) Tt(e);
            e.setupObserver('resource', Tt);
          })(),
        0
      ),
      setTimeout(
        () =>
          (function () {
            ...
            for (const e of t) Nt(e);
            e.setupObserver('measure', Nt);
          })(),
        0
      ));
  }
  function Nt(e) {
    const t = e.name;
    t.substring(0, De.length) !== De && St.createUserTimingTrace(t);
  }
  /**
   * @license
   * Copyright 2019 Google LLC
   ...
   */ class Ot {
      ...
      (this.app = e),
        He.getInstance().requiredApisAvailable() &&
          new Promise((e, t) => {
            ...
            .then(e => {
              e && (gt || (mt(5500), (gt = !0)), ut().then(At, At));
            })
            .catch(e => {
              Ue.info("Environment doesn't support IndexedDB: " + e);
            });
    }
    trace(e) {
      return new St(e);
    ...
    get instrumentationEnabled() {
      return ze.getInstance().instrumentationEnabled;
    }
    set dataCollectionEnabled(e) {
      ze.getInstance().dataCollectionEnabled = e;
    }
   ...
   */ !(function (e) {
    const t = (e, t) => {
      if ('[DEFAULT]' !== e.name) throw $e.create('FB not default');
      if ('undefined' == typeof window) throw $e.create('no window');
      return (
        (function (e) {
          xe = e;
        })(window),
        (ze.getInstance().firebaseAppInstance = e),
        (ze.getInstance().installationsService = t),
        new Ot(e)
      );
    };
    e.INTERNAL.registerComponent(
      new s(
        ...
        e => {
          const n = e.getProvider('app').getImmediate(),
            r = e.getProvider('installations').getImmediate();
          return t(n, r);
        },
        'PUBLIC'
      )
    ),
      e.registerVersion('@firebase/performance', Ce);
  })(A);
  /**
   * @license
   * Copyright 2019 Google LLC
